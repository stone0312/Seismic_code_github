...
!================================================================
     ...
     +      IRECP)
      ...
      REAL    CO_TP_SEMB1(LT,NP),CO_TP_SEMB_M1(LT,NP)
...
!        DO 8888 icenter=1,NX   !NX_H+1, Nbeam_ALL, NX_ALL
        DO 8888 icenter=100,101   !NX_H+1, Nbeam_ALL, NX_ALL
!         if(mod(icenter,100).eq.0) 
         print*,icenter,nx
!        icenter=396
        DO IX=1,Nbeam
          DO IT=1,LT
...
!inversion forward tau-p result
        open(55,file='orig_1st_CO_TP_LOCAL_icenter100',
...
!initial sembling cofficients 			
        open(55,file='CO_TP_SEMB_icenter20',
     +  access='direct',recl=lt)
        do ix=1,NP
         write(55,rec=ix)(CO_TP_SEMB(it,ix),it=1,lt)
        enddo
        close(55)
!        open(55,file='CO_TP_SEMB_M',
!     +  access='direct',recl=lt)
!        do ix=1,NP
...
!        close(55)
      ...
      DO IP=1,NP
        DO IT=1,LT
           if(CO_TP_SEMB(IT,IP).GE.0.2)THEN

              CO_TP_SEMB1(IT,IP)=1.0
              CO_TP_SEMB(IT,IP)= CO_TP_SEMB(IT,IP)
            else 
              CO_TP_SEMB1(IT,IP)=0.0
      ...
      DO IP=1,NP
        DO IT=1,LT
          if(CO_TP_SEMB_M(IT,IP).GE.0.2)THEN
             CO_TP_SEMB_M1(IT,IP)=1.0
             CO_TP_SEMB_M(IT,IP)= CO_TP_SEMB_M(IT,IP)
          else
             CO_TP_SEMB_M1(IT,IP)=0.0
       ...
       DO IP=1,NP
        DO IT=1,LT
          CO_TP_LOCAL(IT,IP)=CO_TP_LOCAL(IT,IP)
     +      *CO_TP_SEMB1(IT,IP)
        ENDDO
       ENDDO

       open(55,file='thre02_TP_LOCAL_icenter100.dat',
     + access='direct',recl=lt)
       do ix=1,NP
         write(55,rec=ix)(CO_TP_LOCAL(it,ix),it=1,lt)
       enddo
       close(55)
        ...
        open(55,file='thre01_CO_TP_SEMB_icenter150.dat',
     +  access='direct',recl=lt)
        do ix=1,NP
         write(55,rec=ix)(CO_TP_SEMB(it,ix),it=1,lt)
        enddo
        close(55)
        open(55,file='thre01_CO_TP_SEMB_M_icenter150.dat',
     +  access='direct',recl=lt)
        do ix=1,NP
          write(55,rec=ix)(CO_TP_SEMB_M(it,ix),it=1,lt)
        enddo
      ...
      DO IP=1,NP
        DO IT=1,LT
          CO_TP_SEMB(IT,IP)=CO_TP_SEMB(IT,IP)*abs(CO_TP_SEMB1(IT,IP)
     +     -CO_TP_SEMB_M1(IT,IP))
!          CO_TP_SEMB(IT,IP)=CO_TP_SEMB1(IT,IP)-CO_TP_SEMB_M(IT,IP)
       ENDDO
      ENDDO
!      DO IP=1,NP
!       DO IT=801,LT
!         CO_TP_SEMB(IT,IP)=CO_TP_SEMB(IT,IP)-0.5*CO_TP_SEMB(IT,IP)
!     +     *CO_TP_SEMB_M(IT,IP)
!        ENDDO 
!      ENDDO

!initial substracted sembling cofficients
        open(55,file='thre02_CO_TP_SEMB_INI_SUBSTRACTED_icenter
     +20.dat',access='direct',recl=lt)
        do ix=1,NP
        write(55,rec=ix)(CO_TP_SEMB(it,ix),it=1,lt)
      enddo
      close(55)
...
!       open(55,file='thre04_TP_LOCAL_icenter20.dat',
!     + access='direct',recl=lt)
!       do ix=1,NP
!         write(55,rec=ix)(CO_TP_LOCAL(it,ix),it=1,lt)
!       enddo
...
!CCO_TP_LOCAL:SUBSTRACTED FORWARD TAU-P RESULT FREQUENCY DOMAIN
     ...
     +       plan_f,plan_b)
        ...
        DO IX=icenter-NX_H, icenter+NX_H
          ...
          DO IT=1,800
             CO_ALL_R(IT,IX)=1.0*CO_LOCAL(IT,ilocal)
...
!      close(55)
     ...
     +status='replace')
      do ix=1,nx
        write(55,rec=ix)(CO_ALL_R(it,ix),it=1,lt)
      enddo
      close(55)
!			stop
...
!=====smooth
      ...
      DO IP=1,NP
        DO IT=1,LT
          ...
          enddo
          CO_TP_SEMB_SM(IT,IP)=sum1/819.0
        ENDDO
      ENDDO
...
!      print*,'LT, DT, DX, NW1, NW2, NW3, NW4, NW'
!      print*,LT, DT, DX, NW1, NW2, NW3, NW4, NW
!      print*,'DP, DW, Nbeam_h, NP_h, Nbeam, NP'
!      print*,DP, DW, Nbeam_h, NP_h, Nbeam, NP
...
!==============================================================
     ...
     +     (CO_ALL_M(IT,IX),IT=1,LT)
        ...
        DO IT=1,LT
           CO_ALL_M(IT,IX)=CO_ALL_M(IT,IX)*1000000
        ENDDO
...
!==================================================================
     ...
     +   Nbeam_h, NP_h, NS_START, NS_END)
      ...
      OPEN(11,FILE='new_10_beam_RLSI_CS_multiple_sub_v1_many_shots_mpi
     +.par')
