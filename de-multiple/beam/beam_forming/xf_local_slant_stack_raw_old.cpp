...
int cal_space_spectrum(complex<float> **dxf, complex<float> **psd, complex<float> **dxf1, complex <float> *base,complex <float> *base1, complex<float>*base_acs, complex<float>**acs, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
{
    int ix, it, ip, ix1;
   ...
   for(it=ifmin;it<ifmax;it++)
     {
       ...
       for(ix=0;ix<nx;ix++)
         for(ix1=0;ix1<nx;ix1++)
           {
              acs[ix][ix1].real()/=(float)nx;
              acs[ix][ix1].imag()/=(float)nx;
           }
...
int CLEAN(complex<float> **upf_new, float rou, int ite_max, float **psd_amp, complex<float> **upf, complex <float> *base2,  complex <float> *base22, complex <float> *base3, complex <float> *base4, float *amp_tmp, complex<float> **dxf, complex<float> **psd, complex<float> **dxf1,complex<float> **acsk,complex<float> *base_acsk, float *ratio, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
{
    int ir,ix,ix1,ip,it, ite,ipmax,sign;
    int ite_tmp;
    complex<float> spek_tmp;
    float tmp1,tmp2;

    for(ix=0;ix<nx;ix++)
     for(it=ifmin;it<ifmax;it++)
        tmp1+=(pow(psd[ix][it].real(),2)+pow(psd[ix][it].imag(),2));

    if(tmp1==0.0)
      {
        for(it=0;it<ifmax-ifmin;it++)
           ratio[it]=0.0;
      }
    else
      {
       for(it=0;it<ifmax-ifmin;it++)
         {
          tmp2=0.0;
          for(ix=0;ix<nx;ix++)
             tmp2+=(pow(psd[ix][it+ifmin].real(),2)+pow(psd[ix][it+ifmin].imag(),2));
          ratio[it]=tmp2/tmp1;  
         }
      } 
/* 
    for(it=0;it<ifmax-ifmin;it++)
      cout<<it+ifmin<<" ,  "<<ratio[it]<<endl;
    return 0;
*/
...
//         if(ratio[it-ifmin]>pow(10,-5))
         if(ratio[it-ifmin]>0.0)
          {//1111
           for(ite=0;ite<ite_max;ite++)
            {
              ...
              for(ip=0;ip<np;ip++)
                ...
                for(ix=0;ix<nx;ix++)
                 {
                   base22[ix].real()=0.0;
                   base22[ix].imag()=p[ipmax]*omega[it]*x[ix];
                   base22[ix]=exp(base22[ix]);
                 }
              ...
              for(ix=0;ix<nx;ix++)
                for(ix1=0;ix1<nx;ix1++)
                 {
                   acsk[ix][ix1].real()/=(float)nx;
                   acsk[ix][ix1].imag()/=(float)nx;
                 }
               
              for(ix=0;ix<nx;ix++)
                base_acsk[ix]=(0.0,0.0);
              spek_tmp=(0.0,0.0);

              for(ix=0;ix<nx;ix++)
                {
                  for(ix1=0;ix1<nx;ix1++)  
                    base_acsk[ix]+=base22[ix1]*acsk[ix1][ix];
                }
 
              for(ix=0;ix<nx;ix++)
                 spek_tmp+=base_acsk[ix]*base2[ix];
                ...
                for(ip=0;ip<np;ip++)
                  {
                    ...
                    for(ix=0;ix<nx;ix++)
                     ...
                     amp_tmp[ip]=rou*sqrt(pow(spek_tmp.real(),2)+pow(spek_tmp.imag(),2))/nx/nx*(pow(base4[ip].real(),2)+pow(base4[ip].imag(),2));
                  }
               ...
               if(sign==np)
                 {
                    ...
                    for(ip=0;ip<np;ip++)
                      { 
                        psd[ip][it].real()-=spek_tmp.real()*rou/nx/nx*(pow(base4[ip].real(),2)+pow(base4[ip].imag(),2));
                        psd[ip][it].imag()-=spek_tmp.imag()*rou/nx/nx*(pow(base4[ip].real(),2)+pow(base4[ip].imag(),2));
                      }
                 }
               else
                ite+=ite_max+999;
...
int main()
{
   char fn1[256],fn2[256],fn3[256],fn4[256],fn5[256],fn6[256];
   int ns, nr, lt;
   float dx, dt;
   int nx_h, np_h;
   float fmin, fmax,theta_max,theta_min,v, rou;
   int ite_max;

   int is,ir,ix,ip,it;

   ifstream swq;
   swq.open("xf_local_slant_stack_raw.par");
   if(!swq)
     {
        cout<<"Cannot open xf_local_slant_stack_raw.par"<<endl;
        return 0;
     } 
   swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn6>>ns>>nr>>lt>>dx>>dt>>nx_h>>np_h>>fmin>>fmax>>theta_max>>theta_min>>v>>rou>>ite_max;
   swq.close();
...
*/
   ...
   complex<float> **psd;
   psd=alloc2complex(lt,np);
       
   float **upf_amp;
   upf_amp=alloc2float(lt,np);

   float **psd_amp;
   psd_amp=alloc2float(lt,np);
...
//        for(ir=0;ir<1;ir++)
          {
           ...
           cal_space_spectrum(uf, psd, uf1, base,base1, base_acs, acs, omega, p, x, nx,  np,  lt,  ifmin,  ifmax);
           ...
           CLEAN(upf_new, rou, ite_max, psd_amp, upf, base2,base22, base3, base4, amp_tmp,uf, psd, uf1,acsk, base_acsk, ratio, omega, p, x, nx,  np,  lt,  ifmin,  ifmax);
