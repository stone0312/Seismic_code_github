...
int CLEAN(complex<float> **upf_new, float rou, int ite_max, float **psd_amp, complex<float> **upf, complex <float> *base2, complex <float> *base3, complex <float> *base4, complex<float> **dxf, complex<float> **psd, complex<float> **dxf1, complex <float> *base,complex <float> *base1, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
{
    int ir,ix,ix1,ip,it, ite,ipmax;
    float amp_tmp;

//    xf_local_tau_p(dxf, upf, base, omega, p, x,  nx,  np, lt, ifmin, ifmax);
 
//int cal_space_spectrum(complex<float> **dxf, complex<float> **psd, complex<float> **dxf1, complex <float> *base,complex <float> *base1, complex<float>*base_acs, complex<float>**acs, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)

//    cal_space_spectrum(dxf, psd, dxf1, base,base1, base_acs, acs, omega, p, x, nx,  np,  lt,  ifmin,  ifmax);  
          ...
          for(ite=0;ite<ite_max;ite++)
            {
              for(ip=0;ip<np;ip++)
                ...
                for(ip=0;ip<np-1;ip++)
                  {
                     if(psd_amp[ip+1][it]>psd_amp[ip][it])
                       ipmax=ip+1;                        
                  }
                    ...
                    for(ix=0;ix<nx;ix++)
                     {
                       ...
                       base3[ix]=exp(base[ix]);
                     }
                    for(ix=0;ix<nx;ix++)
                       base4[ip]+=base2[ix]*base3[ix];

                     amp_tmp=rou*psd_amp[ip][it]/nx/nx*(pow(base4[ip].real(),2)+pow(base4[ip].imag(),2));
                   
                     if(psd_amp[ip][it]<amp_tmp)
                        ite=ite_max+999;
                     else
                       {
                         psd[ip][it].real()-=psd[ip][it].real()*rou/nx/nx*(pow(base4[ip].real(),2)+pow(base4[ip].imag(),2));
                         psd[ip][it].imag()-=psd[ip][it].imag()*rou/nx/nx*(pow(base4[ip].real(),2)+pow(base4[ip].imag(),2));
                       }
...
//int CLEAN(complex<float> **upf_new, float rou, int ite_max, float **psd_amp, complex<float> **upf, complex <float> *base2, complex <float> *base3, complex <float> *base4, complex<float> **dxf, complex<float> **psd, complex<float> **dxf1, complex <float> *base,complex <float> *base1, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
           CLEAN(upf_new, rou, ite_max, psd_amp, upf, base2, base3, base4, uf, psd, uf1, base,base1,  omega, p, x, nx,  np,  lt,  ifmin,  ifmax);
