...
int cal_space_spectrum(complex<float> **dxf, complex<float> **psd, complex<float> **dxf1, complex <float> *base,complex <float> *base1, complex<float>*base_acs, complex<float>**acs, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax, float * psd_amp_tmp, float **filter)
{
    int ix, it, ip, ix1, it1;
    float amp_filter;

    ofstream swq88;
   swq88.open("./2dfield_psd_amp_snap_pw_filter_trace150_nx21.dat",ios::binary);
   if(!swq88)
     {
...
int CLEAN(complex<float> **upf_new, float rou, int ite_max, float **psd_amp, complex<float> **upf, complex <float> *base2,  complex <float> *base22, complex <float> *base3, complex <float> *base4, float *amp_tmp, complex<float> **dxf, complex<float> **psd, complex<float> **dxf1,complex<float> **acsk,complex<float> *base_acsk,  float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
{
    ...
    int ite_tmp;
    float amp_raw,amp_new;

   ofstream swq888;
   swq888.open("./2dfield_psd_amp_snap_pw_fre100_ite_trace150_nx21.dat",ios::binary);
   if(!swq888)
     {
       cout<<"cannot open "<<endl;
       return 0;
...
//    for(it=79;it<80;it++)
       {
        ...
        amp_raw=0.0;
        for(ip=0;ip<np;ip++)
          amp_raw+=sqrt(pow(psd[ip][it].real(),2)+pow(psd[ip][it].imag(),2));  
           ...
           for(ite=0;ite<ite_max;ite++)
            {
              ...
              amp_new=0.0;
              for(ip=0;ip<np;ip++)
                amp_new+=psd_amp[ip][it];

            if(amp_new!=0.0)
             {//88
              cout<<"   ===="<<ite+1<<" ,  "<<amp_new/amp_raw<<endl; 
              if(amp_new/amp_raw<0.05)
                  ite+=999;
              else
               {//888
               if(it==98)
                {
                  for(ip=0;ip<np;ip++)
                   swq888.write((char*)&psd_amp[ip][it],sizeof(psd_amp[ip][it]));
                }
...
int main()
{
   ...
   swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn6>>fn7>>fn8>>fn9>>ns>>nr>>lt>>dx>>dt>>nx_h>>np_h>>fmin>>fmax>>theta_max>>theta_min>>v>>rou>>ite_max>>thre;
   swq.close();
        ...
        for(ir=149;ir<150;ir++)
          {
            ...
            for(ix=0;ix<ir;ix++)
               swq9.seekg(np*lt*4,ios::cur);

            for(ip=0;ip<np;ip++)
              for(it=0;it<lt;it++)
               swq9.read((char*)&sem1[ip][it],sizeof(sem1[ip][it]));
           ...
           cal_space_spectrum(uf, psd, uf1, base,base1, base_acs, acs, omega, p, x, nx,  np,  lt,  ifmin,  ifmax, psd_amp_tmp, sem);
