...
int cal_space_spectrum(complex<float> **dxf, complex<float> **psd, complex<float> **dxf1, complex <float> *base,complex <float> *base1, complex<float>*base_acs, complex<float>**acs, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax, float * psd_amp_tmp)
{
    int ix, it, ip, ix1;

    ofstream swq88;
   swq88.open("./psd_amp_snap_pw.dat",ios::binary);
   if(!swq88)
     {
        ...
        for(ir=100;ir<101;ir++)
          {
           ...
           cal_space_spectrum(uf, psd, uf1, base,base1, base_acs, acs, omega, p, x, nx,  np,  lt,  ifmin,  ifmax, psd_amp_tmp);
