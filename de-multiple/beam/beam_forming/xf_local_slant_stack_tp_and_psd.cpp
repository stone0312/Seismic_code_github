...
int main()
{
   ...
   ifstream swq;
   swq.open("xf_local_slant_stack_tp_and_psd.par");
   if(!swq)
     {
        cout<<"Cannot open xf_local_slant_stack.par"<<endl;
        return 0;
     } 
   swq>>fn1>>fn2>>fn3>>fn4>>ns>>nr>>lt>>dx>>dt>>nx_h>>np_h>>fmin>>fmax>>theta_max>>theta_min>>v;
   swq.close();
...
//int xf_local_tau_p(complex<float> **dxf, complex<float> **dpf, complex <float> *base, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
             ...
             for(ip=0;ip<np;ip++)
               for(it=0;it<lt;it++)
                  upf_amp[ip][it]=pow(upf[ip][it].real(),2)+pow(upf[ip][it].imag(),2);
...
//int cal_space_spectrum(complex<float> **dxf, complex<float> **psd, complex<float> **dxf1, complex <float> *base,complex <float> *base1, complex<float>*base_acs, complex<float>**acs, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
           ...
           cal_space_spectrum(uf, psd, uf1, base,base1, base_acs, acs, omega, p, x, nx,  np,  lt,  ifmin,  ifmax);
             ...
             for(ip=0;ip<np;ip++)
               for(it=0;it<lt;it++)
                  psd_amp[ip][it]=pow(psd[ip][it].real(),2)+pow(psd[ip][it].imag(),2);
