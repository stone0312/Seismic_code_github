...
int CLEAN(complex<float> **upf_new, float rou, int ite_max, float **psd_amp, complex<float> **upf, complex <float> *base2, complex <float> *base3, complex <float> *base4, complex <float> *base5,complex<float> **psd, complex<float>*base_acs1, complex<float>**acs1, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
{
    int ir,ix,ix1,ip,it, ite,ipmax;
    float amp_tmp;
    complex<float> tmp;
    ...
    for(it=ifmin;it<ifmax;it++)
       {
          ...
          amp_tmp=0.0;
          for(ip=0;ip<np;ip++)
            amp_tmp+=sqrt(pow(psd[ip][it].real(),2)+pow(psd[ip][it].imag(),2)); 
          cout<<it<<"===="<<amp_tmp<<endl;

          if(amp_tmp<pow(10,-15)) 
            {
               for(ip=0;ip<np;ip++)
                 upf_new[ip][it]=(0.0,0.0);
            }  
            ...
            {//1111
              for(ip=0;ip<np;ip++)
                ...
                for(ix=0;ix<nx;ix++)
                 {
                   base3[ix].real()=0.0;
                   base3[ix].imag()=-p[ipmax]*omega[it]*x[ix];
                 ...
                 for(ix=0;ix<nx;ix++)
                   {
                      for(ix1=0;ix1<nx;ix1++)
                        acs1[ix][ix1]=base2[ix]*base3[ix1];
                   }
                 for(ix=0;ix<nx;ix++)
                   for(ix1=0;ix1<nx;ix1++)
                      acs1[ix][ix1]*=psd[ipmax][it];
 
                  for(ix=0;ix<nx;ix++)
                   for(ix1=0;ix1<nx;ix1++)
                    {
                      acs1[ix][ix1].real()/=(float)pow(nx,2);
                      acs1[ix][ix1].imag()/=(float)pow(nx,2);
                    }

                for(ip=0;ip<np;ip++)
                  {
                   if(psd_amp[ip][it]!=0.0)
                   {
                    tmp=(0.0,0.0);  

                    for(ix=0;ix<nx;ix++) 
                       base_acs1[ix]=(0.0,0.0);

                    for(ix=0;ix<nx;ix++)
                     {
                       base4[ix].real()=0.0;
                       base4[ix].imag()=-p[ip]*omega[it]*x[ix];
                       base4[ix]=exp(base4[ix]);
                     }
                    for(ix=0;ix<nx;ix++)
                     {
                       base5[ix].real()=0.0;
                       base5[ix].imag()=p[ip]*omega[it]*x[ix];
                       base5[ix]=exp(base5[ix]);
                     }

                     for(ix=0;ix<nx;ix++)
                       {
                        for(ix1=0;ix1<nx;ix1++)
                           base_acs1[ix]+=base4[ix1]*acs1[ix1][ix]; 
                       }

                     for(ix=0;ix<nx;ix++)
                        tmp+=base_acs1[ix]*base5[ix];
 
                     if((psd_amp[ip][it]-sqrt(pow(rou*tmp.real(),2)+pow(rou*tmp.imag(),2)))/psd_amp[ip][it]>0.01)
  //                   if(psd_amp[ip][it]-sqrt(pow(rou*tmp.real(),2)+pow(rou*tmp.imag(),2))>0.0)
                       {
                         psd[ip][it].real()-=rou*tmp.real();  
                         psd[ip][it].imag()-=rou*tmp.imag();  
                       }
                     else
                        ite=ite_max+9999;
                   }  
...
*/
   ...
   complex<float> *base4;
   base4=alloc1complex(nx); 
   complex<float> *base5;
   base5=alloc1complex(nx); 

   complex<float> *base_acs;
   base_acs=alloc1complex(nx); 
   complex<float> *base_acs1;
   base_acs1=alloc1complex(nx); 

   complex<float> **acs;
   acs=alloc2complex(nx,nx);
   complex<float> **acs1;
   acs1=alloc2complex(nx,nx);
...
//int CLEAN(complex<float> **upf_new, float rou, int ite_max, float **psd_amp, complex<float> **upf, complex <float> *base2, complex <float> *base3, complex <float> *base4, complex <float> *base5,complex<float> **psd, complex<float>*base_acs1, complex<float>**acs1, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax)
         ...
         CLEAN(upf_new, rou, ite_max, psd_amp, upf, base2, base3, base4, base5, psd, base_acs1, acs1, omega, p, x, nx,  np,  lt,  ifmin,  ifmax);
