...
!      DO 6666 ISHOT=1,NSHOT
       DO 6666 ISHOT=200,200 
        ...
        L25=L25+NP*LT                   !SMOOTHING SUBS
	                                      !+SEMBLING:CO_TP_SEMB_SM
        PRINT*,'MEMORY USED:',REAL(L25)/256.0/1024.0,'Mb'

        ALLOCATE(BUFF(L25),STAT=IERR)
        IF(IERR.NE.0.0)THEN
        PRINT*,'ALLOCATE ERR'
        STOP
        ENDIF
     ...
     +         BUFF(L21), BUFF(L22), BUFF(L23), BUFF(L24),
     +         INDEX_TABLE,
     +         LT, DT, NX, DX,
     +         NW1, NW2, NW3, NW4, NW,
...
!================================================================
     ...
     +      R1, R2, P1, P2, AP, ENERGY, HEAD_ALL, INDEX_TABLE,
     +      LT, DT, NX, DX,
     +      NW1, NW2, NW3, NW4, NW,
     +      DP, DW, Nbeam_h, NP_h, Nbeam, NP, Nbeam_ALL,
     +      ISHOT, NSHOT,
...
!inversion forward tau-p result
       open(55,file='CO_TP_LOCAL_shot200_icenter200_smt9',
...
!initial sembling cofficients 			
       open(55,file='CO_TP_SEMB_shot200_icenter200_smt9',
     + access='direct',recl=lt)
      do ix=1,NP
        write(55,rec=ix)(CO_TP_SEMB(it,ix),it=1,lt)
      enddo
      close(55)
      open(55,file='CO_TP_SEMB_M_shot200_icenter200_smt9',
     +access='direct',recl=lt)
      do ix=1,NP
...
!       open(55,file='CO_TP_SEMB_INI_SUBSTRACTED_shot200_icenter200_smt9',
!     + access='direct',recl=lt)
!      do ix=1,NP
!        write(55,rec=ix)(CO_TP_SEMB(it,ix),it=1,lt)
!      enddo
...
!smoothing substracted sembling cofficients
       open(55,file='CO_TP_SUBS_SEMB_SM_shot200_icenter200_smt9',
     + access='direct',recl=lt)
       do ix=1,NP
...
!  		open(55,file='CO_TP_LOCAL_shot400_icenter200_smt9',
!     + access='direct',recl=lt)
!      do ix=1,NP
!        write(55,rec=ix)(CO_TP_LOCAL(it,ix),it=1,lt)
!      enddo
...
!CCO_TP_LOCAL:SUBSTRACTED FORWARD TAU-P RESULT FREQUENCY DOMAIN
     ...
     +       plan_f,plan_b)
        ...
        DO IX=icenter-NX_H, icenter+NX_H
          ...
          DO IT=1,LT
             CO_ALL_R(IT,IX)=10*CO_LOCAL(IT,ilocal)
          ENDDO
          print*,ix,ilocal
          ilocal=ilocal+1
          ENDIF
...
!=====smooth
        ...
        DO IT=1,LT
          ...
          enddo
          CO_TP_SEMB_SM(IT,IP)=sum1/369.0
        ENDDO
      ENDDO
...
!==================================================================
     ...
     +   Nbeam_h, NP_h, NS_START, NS_END)
      ...
      OPEN(11,FILE='beam_RLSI_CS_multiple_sub.par')
