...
int main()
{
  ...
  ifstream swq;
  swq.open("weight_estimation_MCG_forward_primary_estimation.par");
  swq>>fn1>>fn2>>fn111>>fn222>>fn3>>fn4>>fn9>>fn10>>fn5>>fn6>>fn7>>fn8>>fn11>>fn12>>ns>>nr>>lt>>dt>>fmin>>fmax>>lamda>>coe1>>l;
  swq.close();
  ...
  cout<<"Totally "<<ifmax-ifmin+1<<" Frequency Slices Needed to be Calculated..."<<endl;
   ...
   float *ham;
   ham=alloc1float(ifmax);

   for(it=0;it<ifmax;it++)
      ham[it]=1.0;
/*
   for(it=ifmin;it<(ifmin+len1);it++)
      ham[it]=sqrt(sin(pai*(it-ifmin)/(2*(len1))));
   for(it=ifmax-len1;it<ifmax;it++)
      ham[it]=ham[ifmin+ifmax-it-1];
...
*/
      ...
      for(ir=0;ir<nr;ir++)
       for(it=0;it<ifmax;it++)
         {
            pf2d[ir][it].real()*=ham[it];
            pf2d[ir][it].imag()*=ham[it];
         }
