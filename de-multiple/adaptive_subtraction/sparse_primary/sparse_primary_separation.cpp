...
int forward_xf_local_tau_p(float **hrtp, float **dxt, complex<float> **dxf, complex<float> **dpf, float **dpt,  complex <float> *base, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax, float **sem, float threhold, int ite_max, int pw_half, int tw_half, int thalf, float **sem_tmp, float *window)
{
   int ix, it, ip;
   int ite;
         ...
         if(sem_max<threhold)
         ...
         else
           {
             for(it=t_idx-thalf;it<t_idx+thalf+1;it++)
                hrtp[p_idx][it]=dpt[p_idx][it]*window[it-t_idx+thalf];
...
//         wm[ip][it]=fabs(p0tp[ip][it])*lamda2/2/(1+eta);
         wp[ip][it]=fabs(m0tp[ip][it]);
         wm[ip][it]=fabs(p0tp[ip][it]);
       }
...
//calculate A(ATpt) and A(ATmt), forward of the inverse results
            forward_xf_local_tau_p(hraatptp, atpt, atptf1, aatptpf, aatptp ,  base, omega, p, x, nx , np, lt, ifmin, ifmax, sem_d, threhold, ite_max, pw_half, tw_half, thalf, sem_tmp, window);
            forward_xf_local_tau_p(hraatmtp, atmt, atmtf1, aatmtpf, aatmtp ,  base, omega, p, x, nx , np, lt, ifmin, ifmax, sem_m, threhold, ite_max, pw_half, tw_half, thalf, sem_tmp, window);

            for(ip=0;ip<np;ip++)          
              for(it=0;it<lt;it++)
                {
//Data-Matching in the x-t domain
//                   ptp_tmp[ip][it]=dtp[ip][it]-hraatmtp[ip][it]-hraatptp[ip][it]+ptp[ip][it];    
//                   mtp_tmp[ip][it]=mtp[ip][it]+m0tp[ip][it]-eta*hraatptp[ip][it]-(1+eta)*hraatmtp[ip][it]+eta*dtp[ip][it];    

//Data-Matching in the Tau-P domain
                   ptp_tmp[ip][it]=ptp[ip][it];    
                   mtp_tmp[ip][it]=m0tp[ip][it]-eta*(ptp[ip][it]+mtp[ip][it]-dtp[ip][it]);    
                }          
...
int main()
{
    ...
    int nshot,ntrace,np,lt,N,n,nhalf, ite_max, pw_half, tw_half, thalf,taper;
    ...
    int win_time;
     ...
     swq.open("sparse_primary_separation.par");
      if(!swq)
       ...
       swq>>input1>>input11>>output1>>output2>>output3>>output11>>output22>>output33>>output4>>output9>>output10>>output111>>output12>>output13>>output14>>output15>>output16>>output17>>outputwp>>outputwm>>outputsnap1>>outputsnap2>>outputsnap3>>outputsnap4>>nshot>>ntrace>>lt>>np>>N>>theata_max>>theata_min>>fmin>>fmax>>dt>>dx>>v>>threhold1>>threhold>>win_time>>epsilon>>n>>nhalf>>lamda1>>lamda2>>eta>>ite_max>>pw_half>>tw_half>>thalf>>taper;
     swq.close();
...
//atptf, ptpf, atpt, atmtf, mtpf, atmt,  aatptpf, atptf1, aatptp, aatmtpf, atmtf1, aatmtp
  ...
  float **hraatptp =new float *[np];
   for(ix=0;ix<np;ix++)
    hraatptp[ix]=new float [lt];
  float **hraatmtp =new float *[np];
   for(ix=0;ix<np;ix++)
    hraatmtp[ix]=new float [lt];
...
*/
                 ...
                 forward_xf_local_tau_p(hr_u_ini_for, ini_local, dxf, dpf, u_ini_for ,  base, omega, p, x, N , np, lt, ifmin, ifmax,sem_ini, threhold1, ite_max, pw_half, tw_half, thalf, sem_tmp, window);

                 for(ip=0;ip<np;ip++)
                   for(it=0;it<lt;it++)
                      swq5.write((char*)&hr_u_ini_for[ip][it],sizeof(hr_u_ini_for[ip][it]));

                 inverse_xf_local_tau_p(hr_u_ini_for, u_final_f, u_ini_for_f, ufinal , base_T, omega, p, x,  N,  np,  lt,  ifmin, ifmax);
              ...
              for(ix=0;ix<N;ix++)
                for(it=0;it<lt;it++)
                 ...
                 forward_xf_local_tau_p(hr_m_ini_for, m_local, mxf, mpf, m_ini_for ,  base, omega, p, x, N , np, lt, ifmin, ifmax, sem_m_ini, threhold1, ite_max, pw_half, tw_half, thalf, sem_tmp, window);

                 for(ip=0;ip<np;ip++)
                   for(it=0;it<lt;it++)
                      swq55.write((char*)&hr_m_ini_for[ip][it],sizeof(hr_m_ini_for[ip][it]));

                 inverse_xf_local_tau_p(hr_m_ini_for, m_final_f, m_ini_for_f,  mfinal , base_T, omega, p, x,  N,  np,  lt,  ifmin, ifmax);
          ...
          Soft_Threhold_Interation(pt, mt, ptp, mtp, wp, wm, ptp_tmp, mtp_tmp, p1, m1,  lamda1, lamda2, eta, ite_max, err, p0tp, m0tp, p0t, m0t, hr_u_ini_for, ini_local, N, np, lt, snapshot1, snapshot2, snapshot3, snapshot4, atptf, ptpf, atpt, atmtf, mtpf, atmt, base, base_T, aatptpf, atptf1, aatptp, aatmtpf, atmtf1, aatmtp, ptpf1, ptf1, mtpf1, mtf1, omega, p, x, interc, ifmin, ifmax, sem_ini, sem_m_ini, hraatptp, hraatmtp, pw_half, tw_half, thalf, threhold1, sem_tmp, window);
             ...
             for(it=0;it<lt;it++)
                {
                   ptfinal[it]=pt[N/2][it];
                   mtfinal[it]=mt[N/2][it];
                }
 
               for(it=0;it<lt;it++)
                 {
                   swq16.write((char*)&ptfinal[it],sizeof(ptfinal[it]));
                   swq17.write((char*)&mtfinal[it],sizeof(mtfinal[it]));
                 }
