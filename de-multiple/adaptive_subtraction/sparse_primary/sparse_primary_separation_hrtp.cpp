...
int forward_hr_local_tau_p(float **hrtp, float **dxt, complex<float> **dxf, complex<float> **dpf, float **dpt,  complex <float> *base, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax, float **sem, float threhold, int ite_max, int pw_half, int tw_half, int thalf, float **sem_tmp, float *window, int hrmin, int hrmax, float **sem_opt)
{
  ...
  for(ip=0;ip<np;ip++)
   ...
   for(ip=0;ip<np;ip++)
    ...
    for(ip=0;ip<np;ip++)
      for(it=0;it<tw_half;it++)
        sem_tmp[ip][it]=0.0;
    for(ip=0;ip<np;ip++)
      for(it=lt-tw_half;it<lt;it++)
        sem_tmp[ip][it]=0.0;
   ...
   for(ite=0;ite<ite_max;ite++)
     {
        ...
        for(ip=0;ip<np;ip++)
         ...
         if(t_idx-tw_half>-1&&t_idx+tw_half+1<lt) 
          {
            if(p_idx-pw_half<0)
              {
                  for(ip=0;ip<p_idx+hrmax+1;ip++)
                    for(it=t_idx-thalf;it<t_idx+thalf+1;it++)
                     {
                       hrtp[ip][it]=dpt[ip][it]*window[it-t_idx+thalf];
                       sem_opt[ip][it]=sem_tmp[ip][it];
                     }
...
int main()
{
    ...
    float sem_max;
     ...
     ifstream swq;
     swq.open("sparse_primary_separation_hrtp.par");
      if(!swq)
           cout<<"cannot open para card! "<<endl;
    
       swq>>input1>>input11>>output1>>output2>>output3>>output11>>output22>>output33>>output4>>output9>>output10>>output111>>output12>>output13>>output14>>output15>>output16>>output17>>outputwp>>outputwm>>outputsnap1>>outputsnap2>>outputsnap3>>outputsnap4>>output555>>output5555>>nshot>>ntrace>>lt>>np>>nx_half>>theata_max>>theata_min>>fmin>>fmax>>dt>>dx>>v>>threhold1>>threhold>>win_time>>epsilon>>n>>nhalf>>phalf>>lamda1>>lamda2>>eta>>ite_max>>pw_half>>tw_half>>thalf>>taper>>a1>>a2>>hrmin>>hrmax;
     swq.close();
