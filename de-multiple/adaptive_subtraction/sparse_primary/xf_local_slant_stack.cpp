...
int main()
{
   ...
   float **uext;
   uext=alloc2float(lt,nr+nx+1);

   float **ut;
   ut=alloc2float(lt,nx);
        ...
        for(ir=0;ir<1;ir++)
          {
             for(ix=0;ix<nx;ix++)
                for(it=0;it<lt;it++)
                  ut[ix][it]=uext[ir+ix][it];           

             forward_xf_local_tau_p(ut, uf, upf, upt, base, omega, p, x,  nx,  np, lt, ifmin, ifmax);

            for(ip=0;ip<np;ip++)
             for(it=0;it<lt;it++)
               swq2.write((char*)&upt[ip][it],sizeof(upt[ip][it]));
        
            inverse_xf_local_tau_p(upt, uipf, upf,ui,  base_T, omega, p, x,  nx,  np, lt, ifmin, ifmax);
