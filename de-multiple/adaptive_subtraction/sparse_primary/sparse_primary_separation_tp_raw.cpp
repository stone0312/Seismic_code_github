    ...
    float sem_max;
     ...
     swq.open("sparse_primary_separation_tp.par");
      if(!swq)
       ...
       swq>>input1>>input11>>output1>>output2>>output3>>output11>>output22>>output33>>output4>>output9>>output10>>output111>>output12>>output13>>output14>>output15>>output16>>output17>>outputwp>>outputwm>>outputsnap1>>outputsnap2>>outputsnap3>>outputsnap4>>nshot>>ntrace>>lt>>np>>nx_half>>theata_max>>theata_min>>fmin>>fmax>>dt>>dx>>v>>threhold1>>threhold>>win_time>>epsilon>>n>>nhalf>>lamda1>>lamda2>>eta>>ite_max>>pw_half>>tw_half>>thalf>>taper>>a1>>a2;
     swq.close();
...
//atptf, ptpf, atpt, atmtf, mtpf, atmt,  aatptpf, atptf1, aatptp, aatmtpf, atmtf1, aatmtp
  ...
  complex<float> **atptf1=new complex<float> *[2*nx_half+1];
  for(ix=0;ix<2*nx_half+1;ix++)
  ...
  complex<float> **atmtf1=new complex<float> *[2*nx_half+1];
  for(ix=0;ix<2*nx_half+1;ix++)
 ...
 complex<float> **ptf1=new complex<float> *[2*nx_half+1];
 for(ix=0;ix<2*nx_half+1;ix++)
 ...
 complex<float> **mtf1=new complex<float> *[2*nx_half+1];
 for(ix=0;ix<2*nx_half+1;ix++)
...
//             for(it=t_idx-tlen_tmp;it<t_idx+thalf+1;it++)
             for(it=t_idx-thalf;it<t_idx+thalf+1;it++)
    ...
    cout<<"Initial Primary Optimization Begins ..."<<endl;
   ...
   cout<<"Initial Primary Optimization Done!"<<endl;
