...
int forward_xf_local_tau_p(float **hrtp, float **dxt, complex<float> **dxf, complex<float> **dpf, float **dpt,  complex <float> *base, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax, float **sem, float threhold, int ite_max, int pw_half, int tw_half, int thalf, float **sem_tmp)
{
   int ix, it, ip;
   int ite;
...
//calculate A(ATpt) and A(ATmt), forward of the inverse results
            forward_xf_local_tau_p(hraatptp, atpt, atptf1, aatptpf, aatptp ,  base, omega, p, x, nx , np, lt, ifmin, ifmax, sem_d, threhold, ite_max, pw_half, tw_half, thalf, sem_tmp);
            forward_xf_local_tau_p(hraatmtp, atmt, atmtf1, aatmtpf, aatmtp ,  base, omega, p, x, nx , np, lt, ifmin, ifmax, sem_m, threhold, ite_max, pw_half, tw_half, thalf, sem_tmp);
...
int main()
{
    ...
    int nshot,ntrace,np,lt,N,n,nhalf, ite_max, pw_half, tw_half, thalf;
    ...
    int win_time;
     ...
     swq.open("sparse_primary_separation.par");
      if(!swq)
       ...
       swq>>input1>>input11>>output1>>output2>>output3>>output11>>output22>>output33>>output4>>output9>>output10>>output111>>output12>>output13>>output14>>output15>>output16>>output17>>outputwp>>outputwm>>outputsnap1>>outputsnap2>>outputsnap3>>outputsnap4>>nshot>>ntrace>>lt>>np>>N>>theata_max>>theata_min>>fmin>>fmax>>dt>>dx>>v>>threhold1>>threhold>>win_time>>epsilon>>n>>nhalf>>lamda1>>lamda2>>eta>>ite_max>>pw_half>>tw_half>>thalf;
     swq.close();
...
*/
                 ...
                 forward_xf_local_tau_p(hr_u_ini_for, ini_local, dxf, dpf, u_ini_for ,  base, omega, p, x, N , np, lt, ifmin, ifmax,sem_ini, threhold1, ite_max, pw_half, tw_half, thalf, sem_tmp);
              ...
              for(ix=0;ix<N;ix++)
                for(it=0;it<lt;it++)
                 ...
                 forward_xf_local_tau_p(hr_m_ini_for, m_local, mxf, mpf, m_ini_for ,  base, omega, p, x, N , np, lt, ifmin, ifmax, sem_m_ini, threhold1, ite_max, pw_half, tw_half, thalf, sem_tmp);
          ...
          Soft_Threhold_Interation(pt, mt, ptp, mtp, wp, wm, ptp_tmp, mtp_tmp, p1, m1,  lamda1, lamda2, eta, ite_max, err, p0tp, m0tp, p0t, m0t, hr_u_ini_for, ini_local, N, np, lt, snapshot1, snapshot2, snapshot3, snapshot4, atptf, ptpf, atpt, atmtf, mtpf, atmt, base, base_T, aatptpf, atptf1, aatptp, aatmtpf, atmtf1, aatmtp, ptpf1, ptf1, mtpf1, mtf1, omega, p, x, interc, ifmin, ifmax, sem_ini, sem_m_ini, hraatptp, hraatmtp, pw_half, tw_half, thalf, threhold1, sem_tmp);
