...
int forward_hr_local_tau_p(float **hrtp, float **dxt, complex<float> **dxf, complex<float> **dpf, float **dpt,  complex <float> *base, float *omega, float *p, float *x, int nx, int np, int lt, int ifmin, int ifmax, float **sem, float threhold, int ite_max, int pw_half, int tw_half, int thalf, float **sem_tmp, float *window, int hrmin, int hrmax, float **sem_opt)
{
   int ix, it, ip;
   int ite;
   float sem_max;
  ...
  for(ip=0;ip<np;ip++)
   ...
   for(ip=0;ip<np;ip++)
     for(it=0;it<lt;it++)
        {
           if(sem[ip][it]>threhold)
            {
              hrtp[ip][it]=dpt[ip][it]; 
              sem_opt[ip][it]=sem[ip][it]; 
            }
        }
...
int Smoothing_Semblance(float **semb, float **semb1, int phalf, int nhalf, int np, int lt)
{
   int ip, it, ip1, it1;
   for(ip=0;ip<np;ip++)
     for(it=0;it<lt;it++)
   ...
   for(ip=phalf;ip<np-phalf;ip++)
    {       
      for(it=nhalf;it<lt-nhalf;it++)
         {
          for(ip1=ip-phalf;ip1<ip+phalf+1;ip1++)
            for(it1=it-nhalf;it1<it+nhalf+1;it1++)
              semb1[ip][it]+=semb[ip1][it1];

            semb1[ip][it]/=((2*nhalf+1)*(2*phalf+1));
...
int hilber_envelop( float **env, float **tp, float **for_real, float **for_imag, float **inv_real, float **inv_imag,int np, int lt)
{
   int ix,it;
   for(ix=0;ix<np;ix++)
     for(it=0;it<lt;it++)
        ...
        for(ix=0;ix<np;ix++)
           {
             for(it=0;it<lt;it++)
              ...
              for(it=0;it<lt;it++)
                {
                   for_real[ix][it]=out1[it][0]/(float)lt;
                   for_imag[ix][it]=out1[it][1]/(float)lt;
                }
           }
     }
        ...
        for(ix=0;ix<np;ix++)
           {
             for(it=0;it<lt;it++)
               {
                  in2[it][0]=for_real[ix][it];
                  in2[it][1]=for_imag[ix][it];
               }
              ...
              for(it=0;it<lt;it++)
                {
                   inv_real[ix][it]=out2[it][0];
                   inv_imag[ix][it]=out2[it][1];
                }
           }
     }
  ...
  for(ix=0;ix<np;ix++)
    for(it=0;it<lt;it++)
       env[ix][it]=sqrt(pow(inv_real[ix][it],2)+pow(inv_imag[ix][it],2));
    ...
    float sem_max;
     ...
     swq.open("sparse_primary_separation_tp.par");
      if(!swq)
       ...
       swq>>input1>>input11>>output1>>output2>>output3>>output11>>output22>>output33>>output4>>output9>>output10>>output111>>output12>>output13>>output14>>output15>>output16>>output17>>outputwp>>outputwm>>outputsnap1>>outputsnap2>>outputsnap3>>outputsnap4>>output555>>output5555>>outputenv>>nshot>>ntrace>>lt>>np>>nx_half>>theata_max>>theata_min>>fmin>>fmax>>dt>>dx>>v>>threhold1>>threhold>>win_time>>epsilon>>n>>nhalf>>phalf>>lamda1>>lamda2>>eta>>ite_max>>pw_half>>tw_half>>thalf>>taper>>a1>>a2>>hrmin>>hrmax;
     swq.close();
...
//array for soft threshold interation
   ...
   float **env=new float *[np];
   for(ip=0;ip<np;ip++)
      env[ip]=new float [lt];

   float **for_real=new float *[np];
   for(ip=0;ip<np;ip++)
      for_real[ip]=new float [lt];
   float **for_imag=new float *[np];
   for(ip=0;ip<np;ip++)
      for_imag[ip]=new float [lt];
   float **inv_real=new float *[np];
   for(ip=0;ip<np;ip++)
      inv_real[ip]=new float [lt];
   float **inv_imag=new float *[np];
   for(ip=0;ip<np;ip++)
      inv_imag[ip]=new float [lt];
 ...
 // open input data
   ...
   fstream swqenv;
   swqenv.open(outputenv,ios::binary|ios::out);
     		...
     		for(it=0;it<lt;it++)
	           ...
	           p0tp[ip][it]=0.0;
         	   m0tp[ip][it]=0.0;
                 ...
                 cout<<"Original data Tau-P Begins ..."<<endl;

                 forward_hr_local_tau_p(hr_u_ini_for, ini_local, dxf, dpf, u_ini_for ,  base, omega, p, x, N , np, lt, ifmin, ifmax,sem_ini, threhold1, ite_max, pw_half, tw_half, thalf, sem_tmp, window, hrmin, hrmax,sem1);
                 ...
                 for(ip=0;ip<np;ip++)
                   for(it=0;it<lt;it++)
                      swq7.write((char*)&sem1[ip][it],sizeof(sem1[ip][it]));

                 cout<<"Original data Tau-P Done ! "<<endl;
                ...
                for(it=0;it<lt;it++)
                 ...
                 cout<<"Multiple Model Tau-P Begins ..."<<endl;

                 forward_hr_local_tau_p(hr_m_ini_for, m_local, mxf, mpf, m_ini_for ,  base, omega, p, x, N , np, lt, ifmin, ifmax, sem_m_ini, threhold1, ite_max, pw_half, tw_half, thalf, sem_tmp, window, hrmin, hrmax,sem2);




              hilber_envelop( env, hr_m_ini_for, for_real, for_imag, inv_real, inv_imag, np, lt);
       
                 for(ip=0;ip<np;ip++)
                   for(it=0;it<lt;it++)
                      swqenv.write((char*)&env[ip][it],sizeof(env[ip][it]));
 
              for(ip=0;ip<np;ip++)
                for(it=0;it<lt;it++)
                   sem2[ip][it]*=env[ip][it];
...
/*
                 Smoothing_Semblance(sem_m_ini, sem2,  phalf,  nhalf, np,  lt);
                 for(ip=0;ip<np;ip++)
                   for(it=0;it<lt;it++)
                     {
                        if(sem1[ip][it]==0.0)
...
*/
                 ...
                 for(ip=0;ip<np;ip++)
                   for(it=0;it<lt;it++)
                      swq77.write((char*)&sem2[ip][it],sizeof(sem2[ip][it]));

                 cout<<"Multiple Model Tau-P Done ! "<<endl;

                 for(ip=0;ip<np;ip++)
                   for(it=0;it<lt;it++)
                     { 
                       if(sem1[ip][it]==0.0)
                        hr_m_ini_for[ip][it]=0.0;
                     }
...
/*
   Butterworth(sem1, sem2, semb,  epsilon,  n, np, lt, threhold);
	     
   for(ip=0;ip<np;ip++)
     for(it=0;it<lt;it++)
       swq8.write((char*)&semb[ip][it],sizeof(semb[ip][it]));

//   Smoothing_Semblance(semb, sem3,  phalf,  nhalf, np,  lt);
...
//        	 if(semb[ip][it]!=0.0)
	    ...
	    for(ip=0;ip<np;ip++)
	     for(it=0;it<lt;it++)
        	m0tp[ip][it]=hr_u_ini_for[ip][it]-m0tp[ip][it];
...
*/
	    ...
	    for(ip=0;ip<np;ip++)
     	      for(it=0;it<lt;it++)
	        m0tp[ip][it]=hr_u_ini_for[ip][it]*semb[ip][it];

	    for(ip=0;ip<np;ip++)
	     for(it=0;it<lt;it++)
        	p0tp[ip][it]=hr_u_ini_for[ip][it]-m0tp[ip][it];
