...
int main()
{
  ...
  int ns,nr,lt,iter,iter_max,mcg_thre;
  float dt,fmin,fmax,min,max,lamda1,lamda2;
  int n1,n2;
  float amp1;
  float err=0.000000001;

  ifstream swq;
  swq.open("wavelet_estimation_constraint.par");
  swq>>fn1>>fn2>>fn3>>fn4>>fn11>>fn22>>fn6>>fn7>>fn8>>fn9>>ns>>nr>>lt>>dt>>fmin>>fmax>>min>>max>>lamda1>>iter_max>>n1>>n2>>mcg_thre>>lamda2;
  swq.close();
  ...
  int ltt,ifmin,ifmax,is,ir,ir1,ir2,it,it1,irmin,irmax; 
  ltt=2*lt;
  ...
  for(ir=0;ir<nr;ir++)
    {
       if(ir<mcg_thre)
        ...
        else
         {
           ...
           normalize[ir]=irmax-irmin+1; 
         }
    }
/*
  for(ir=0;ir<nr;ir++)
    cout<<ir+1<<" , "<<normalize[ir]<<endl;  
  return 0;
*/
    ...
    for(it=ifmin;it<ifmax;it++)
     {
       ...
       amp1=0.0;
       for(ir=0;ir<nr;ir++)
         amp1+=pow(b[ir].real(),2)+pow(b[ir].imag(),2);
...
//qualitive constraints on MCG: form Constrained  Matrix A
           for(ir1=0;ir1<nr;ir1++)
             for(ir2=0;ir2<nr;ir2++)
                a[ir1][ir2]=(0.0,0.0);
...
*/
           ...
           for(ir1=0;ir1<nr;ir1++) 
             {
               mfor1d[ir1].real()/=normalize[ir1];
               mfor1d[ir1].imag()/=normalize[ir1];
             }
           ...
           for(ir1=0;ir1<nr;ir1++)
               for(ir2=0;ir2<nr;ir2++)
               {
                 ata[ir1][ir2].real()*=pow(10.0,n2); 
                 ata[ir1][ir2].imag()*=pow(10.0,n2); 
               }

           for(ir1=0;ir1<nr;ir1++)
             ata[ir1][ir1].real()+=lamda2;

           complex_matrix_multiply_vector(atb,at,b,nr,nr);
           
           for(ir1=0;ir1<nr;ir1++)
               {
                 atb[ir1].real()*=pow(10.0,n1); 
                 atb[ir1].imag()*=pow(10.0,n1); 
               }
           ...
           for(ir1=0;ir1<nr;ir1++)
              {
                wi[ir].real()*=(pow(10.0,n1)/pow(10.0,n2));
                wi[ir].imag()*=(pow(10.0,n1)/pow(10.0,n2));
              }
           cout<<it+1<<" Frequency, inverse wavelet is===="<<endl;
           for(ir1=0;ir1<nr;ir1++)
             cout<<wi[ir1]<<"   ";
           cout<<endl;
