...
{
   char fn1[256],fn2[256],fn3[256],fn4[256],fn5[256],fn6[256],fn7[256],fn8[256],fn9[256],fn10[256],fn11[256],fn12[256];
   int nx,lt,np_h,nx_h;
   float dx,dt,v,theta_max,f1,f2,f3,f4,threshold,amp;    

   int is,ir,it,ix,tmp,tmp1,is1,ir1,ir2;
   ...
   ifstream swq;
   swq.open("p_domain_wlrm_prediction_with_sparse_constraints_single_survey.par");
   if(!swq)
     {
        cout<<"Cannot open p_domain_wlrm_prediction_with_sparse_constraints_single_survey.par"<<endl;
        abort();
     } 

   swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn6>>fn7>>fn8>>fn9>>fn10>>fn11>>fn12>>nx>>dx>>lt>>dt>>v>>np_h>>nx_h>>theta_max>>threshold>>f1>>f2>>f3>>f4;
   ...
   cout<<"fna of output crg MCG in time domain is===="<<fn12<<endl;
...
//define the arrays used in the subroutine
  ...
  cout<<"Totally "<<mem<<"MB needed to be allocated..."<<endl;
   ...
   fstream swq_mcg;
   swq_mcg.open(fn12,ios::binary|ios::out);
   if(!swq_mcg)
      {
      ...
      for(ir=0;ir<nx;ir++)
       {//2222
        ...
        for(ir1=0;ir1<np*nx;ir1++)
           for(it=0;it<ltt;it++)
              {
                 ugr_p[ir1][it].real()*=1000;
                 ugr_p[ir1][it].imag()*=1000;
              }    
...
//inverse tau-p transform to get the (is,ir) WLRM Prediction
        for(it=0;it<ltt;it++)
         ...
         for(it=0;it<ltt;it++)
          ...
          if((ir+1)%10==0)
             cout<<is+1<<"  shot, "<<ir+1<<"  trace prediction done..."<<endl;
