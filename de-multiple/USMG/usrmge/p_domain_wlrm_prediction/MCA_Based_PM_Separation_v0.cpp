...
//int cal_semblance(float **u,float **b,float **c,float *p,float *x,int *interc,float **sum1,float **sem,int lt,int np,int N,float theata_max,float theata_min,float dt,float dx,float v,float threhold,int win_time)
   ...
   cal_semblance(ud, tp_tmp1, Sd, pr, x, interc, sum1, sem, lt,np, nx_l1, theta_max,theta_min,dt,dx,v,threshold,win_time);

   cal_semblance(um, tp_tmp2, Sm, pr, x, interc, sum1, sem, lt,np, nx_l1, theta_max,theta_min,dt,dx,v,threshold,win_time);
...
//int MCA_Based_PM_Separation(float *u, float *p, float *m, float *r, float *p_final, float *m_final, float **ud, float **um, float **up, float **Sd, float **Sm, float **Sp, float **udtp, float **umtp, float **uptp, float **r1, float **r2, float **umtp1, complex<float> **cudtp, complex<float> **cumtp, int lt, float dx, float dt, float v, int  np_h, int nx_h, int np, int nx_l, float theta_max, float theta_min, float threshold, float f1, float f2, float f3, float f4, float *pr, float *x, int *interc, float **sum1, int win_time, int it_max, float err, float l2err, int n, float epi, float lamda, float **tp_tmp1, float **tp_tmp2, float **sem)
            ...
            MCA_Based_PM_Separation(u, p, m, r, p_final, m_final, ud, um, up, Sd, Sm, Sp, udtp, umtp, uptp, r1, r2, umtp1, cudtp, cumtp, lt,  dx,  dt, v, np_h, nx_h, np, nx_l, theta_max, theta_min, threshold, f1, f2, f3, f4, pr, x,interc, sum1, win_time, it_max, err, l2err, n, epi, lamda, tp_tmp1,tp_tmp2,sem);
