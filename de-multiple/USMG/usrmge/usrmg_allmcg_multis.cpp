...
int main()
{
   ...
   ifstream swq;
   swq.open("usrmg_allmcg_multis.par");
   if(!swq)
      {
         cout<<"cannot open usrmg_allmcg_multis.par"<<endl;
         abort();
      }

   swq>>fn1>>fn11>>fn2>>fn22>>fn3>>fn33>>fn4>>fn44>>fn5>>fn6>>fn7>>nshot>>ntrace>>lt>>dt>>fmax>>fmin;
   swq.close();
...
//calculate the omega
   ...
   complex<float> *su1;
   su1=alloc1complex(ntrace);

   complex<float> *sd;
   sd=alloc1complex(nsr);
   complex<float> *sd1;
   sd1=alloc1complex(ntrace);
              ...
              for(is=0;is<nshot;is++)
                 {
                     for(ir=0;ir<ntrace;ir++)
                       {
                          su1[ir].real()=su[is*ntrace+ir].real();
                          su1[ir].imag()=su[is*ntrace+ir].imag();
                          sd1[ir].real()=sd[is*ntrace+ir].real();
                          sd1[ir].imag()=sd[is*ntrace+ir].imag();
                       } 
                          ...
                          if(is<ir)
                            {
                             for(ix=is;ix<ir+1;ix++)       
                               srmf[itmp]+=(su1[ix]*rd1[ix]+sd1[ix]*ru1[ix]);    
                           
                             srmf[itmp].real()=srmf[itmp].real()/(ir-is+1);
                             srmf[itmp].imag()=srmf[itmp].imag()/(ir-is+1);
                            }
                          else  
                            {
                             ...
                             srmf[itmp].real()=srmf[itmp].real()/(is-ir+1);
                             srmf[itmp].imag()=srmf[itmp].imag()/(is-ir+1);
                            }
                       }
                   }
              for(is=0;is<nsr;is++)
