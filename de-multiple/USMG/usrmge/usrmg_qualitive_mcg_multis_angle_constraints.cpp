...
int main()
{
   char fn1[256],fn2[256],fn3[256],fn4[256],fn11[256],fn22[256],fn33[256],fn44[256],fn5[256],fn6[256],fn7[256],fn8[256];
   int nshot,nr,lt;
   float dt,fmax,fmin,st_beg,st_end;
   int ltt;
   int ifmax,ifmin,nsr,frnel;
   int is,ir,it,ix,itmp,ir1,ir2;
   float dsx,drx,dz; 
 
   ifstream swq;
   swq.open("usrmg_qualitive_mcg_multis_angle_constraints.par");
   if(!swq)
      {
         cout<<"cannot open usrmg_qualitive_mcg_multis_angle_constraints.par"<<endl;
         abort();
      }

   swq>>fn1>>fn11>>fn2>>fn22>>fn3>>fn33>>fn4>>fn44>>fn5>>fn6>>fn7>>fn8>>nshot>>nr>>dsx>>drx>>dz>>lt>>dt>>fmax>>fmin>>st_beg>>st_end;
   swq.close();
...
//   cout<<"=========="<<endl;
   ...
   float *dep;
   dep=alloc1float(nshot);

   ifstream swqq;
    swqq.open(fn8,ios::binary);
    if(!swqq)
       {
    ...
    for(is=0;is<nshot;is++)
       swqq.read((char*)&dep[is],sizeof(dep[is]));
    swqq.close();

    float theta,fai;
    
   int *s_lo;
   s_lo=alloc1int(nshot);
   int *r_lo;
   r_lo=alloc1int(nshot);
          ...
          if(it<ifmin)
           ...
           else
            {
               ...
               for(is=1;is<nsr;is++)
                ...
                for(is=0;is<nshot;is++)
                  {
                     for(ir=0;ir<nshot-1;ir++)
                       {
                          theta=atan((dep[ir+1]-dep[ir])/dsx); 
                          fai=atan(dep[ir]/((ir-is)*dsx));

                          if(ir+dep[ir]/tan(fai-2*theta)/dsx>=0.0)
                             s_lo[ir]=int(ir+dep[ir]/tan(fai-2*theta)/dsx+0.5);
                          else
                             s_lo[ir]=int(ir+dep[ir]/tan(fai-2*theta)/dsx-0.5);
                       }
                     s_lo[nshot-1]=s_lo[nshot-2]; 
...
//                     cout<<"2222"<<endl;
                     ...
                     for(ir=0;ir<nr;ir++)
                       {
                           for(ir1=0;ir1<nshot-1;ir1++)
                             {
                               theta=atan((dep[ir1+1]-dep[ir1])/dsx); 
                               fai=atan(dep[ir1]/((ir1-ir)*dsx));

                               if(ir1+dep[ir1]/tan(fai-2*theta)/dsx>=0.0)
                                r_lo[ir1]=int(ir1+dep[ir1]/tan(fai-2*theta)/dsx+0.5);
                               else
                                r_lo[ir1]=int(ir1+dep[ir1]/tan(fai-2*theta)/dsx-0.5);
                             }
                           r_lo[nshot-1]=r_lo[nshot-2];
                          ...
                          for(ir1=0;ir1<nshot;ir1++)
                            {
                               for(ir2=0;ir2<nshot;ir2++)
                                  {
                                     if(r_lo[ir2]==s_lo[ir1])
                                        {
                                           frnel=r_lo[ir2];
                                           if(frnel<nshot&&frnel>=0)
                                             {
                                               mcg1=su1[frnel]*rd1[frnel];
                                               srmf[itmp]+=mcg1;
                                             }
                                        } 
                                      else
