            ...
            for(it=ltt/2+1;it<ltt;it++)
              {
                 ...
                 ukfu[ir][it].imag()=-ukfmg[ir][ltt-it].imag();
                 ukfg[ir][it].real()=ukfmg[ir][ltt-it].real();
                 ukfg[ir][it].imag()=-ukfmg[ir][ltt-it].imag();
                 ukfm[ir][it].real()=ukfmg[ir][ltt-it].real();
                 ukfm[ir][it].imag()=-ukfmg[ir][ltt-it].imag();
...
int main()
{
   ...
   swq>>fn1>>fn2>>fn3>>fn4>>ns>>nr>>lt>>dt>>ds>>dr>>sdep>>rdep>>v>>theta>>boul>>bour>>fmax>>fmin>>lamda>>n>>wd;
   swq.close();
