                      ...
                      if(pow(omega[it]/v,2)-pow(kx[ir],2)<0.0)
                      ...
                      else if(pow(omega[it]/v,2)-pow(kx[ir],2)>pow(cos(2*pai*theta/360.0)*omega[it]/v,2))
                        {
                             ...
                             ukfg[ir][it]=ukf[ir][it]/ps4;  //downward field
                                  ...
                                  ps5.real()=0.0;
                                  ps5.imag()=(rdep-sdep)*kz;
                                  ps5=exp(ps5);
                                  ukfm[ir][it]=ukfu[ir][it]*ps5;
      

                                  ps6.real()=0.0;
                                  ps6.imag()=-(rdep-sdep)*kz;
                                  ps6=exp(ps6);
                                  ukfmg[ir][it]=ukfg[ir][it]*ps6;
...
int main()
{
   ...
   ifstream swq;
   swq.open("crg_deghost_plus_ext.par");
   swq>>fn1>>fn2>>fn3>>fn4>>fn5>>ns>>nr>>lt>>dt>>ds>>dr>>sdep>>rdep>>v>>theta>>boul>>bour>>fmax>>fmin>>lamda>>n>>wd;
   swq.close();
   ...
   cout<<"Fna of upward wavefields after source-side deghosting and extrapolation is===="<<fn4<<endl; 
   cout<<"Fna of downward wavefields after source-side deghosting and extrapolation is===="<<fn5<<endl; 
      ...
      for(ir=0;ir<nr;ir++)
        for(it=0;it<lt;it++)
         {  
           ...
           swq4.write((char*)&um[ir][it],sizeof(um[ir][it]));
           swq5.write((char*)&umg[ir][it],sizeof(umg[ir][it]));
         }
