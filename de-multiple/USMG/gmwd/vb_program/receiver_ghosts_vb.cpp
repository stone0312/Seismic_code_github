                 ...
                 else
                   {
                      if(pow(omega[it]/v,2)-pow(kx[ir],2)<0.0)
                      ...
                      else if(pow(omega[it]/v,2)-pow(kx[ir],2)>pow(cos(2*pai*theta/360.0)*omega[it]/v,2))
                        {
                             ...
                             ukfg[ir][ltt-it].real()=ukfg[ir][it].real();
                             ukfg[ir][ltt-it].imag()=-ukfg[ir][it].imag();

                             ps2.real()=0.0;               //phase-shift extrapolating operator to achieve upward field
                             ps2.imag()=-(r0+2*wd-dep[ir1])*kz;
                             ps2=exp(ps2);

                             ukfm[ir][it]=ukf[ir][it]*ps2;   //upward field
...
int main()
{
   ...
   ifstream swq;
   swq.open("receiver_ghosts_vb.par");
   swq>>fn1>>fn2>>fn3>>ns>>nr>>lt>>dt>>ds>>dr>>sdep>>rdep>>v>>theta>>boul>>bour>>fmax>>fmin>>lamda>>n>>wd>>r0;
   swq.close();
  ...
  for(ir=0;ir<nr;ir++)
    {
      swq2.seekg(ir*240+64,ios::beg);
      swq2.read((char*)&gwdep[ir],sizeof(gwdep[ir]));
    }
  swq2.close();
  ...
  for(is=0;is<ns;is++)
    {
       ...
       deghosting_plus_extrapolation(uraw,u, uf, ukf, ukfu, ukfg, ukfm, ukfmg, ufu, ufg, ufm, ufmg,uu,ug, um, umg, mg, omega, kx, sdep, rdep,  ds, dr, nr, lt,  ltt,  boul, bour,  nrb,  v, theta,ifmin, ifmax,  lamda,  flag,wd, r0,dep);

      for(ir=0;ir<nr;ir++)
        for(it=0;it<lt;it++)
           swq3.write((char*)&mg[ir][it],sizeof(mg[ir][it]));
