...
/*
    ...
    ofstream swq22;
    swq22.open("testfft.dat",ios::binary); 
    for(ir=0;ir<nr;ir++)
         for(it=0;it<lt;it++)
       ...
       for(ir=nrb/2+1;ir<nrb;ir++)
          {
             ...
             ukfg[ir][it].real()=ukfg[nrb-ir][it].real();
             ukfg[ir][it].imag()=ukfg[nrb-ir][it].imag();
             ukfm[ir][it].real()=ukfm[nrb-ir][it].real();
             ukfm[ir][it].imag()=ukfm[nrb-ir][it].imag();
             ukfmg[ir][it].real()=ukfmg[nrb-ir][it].real();
             ukfmg[ir][it].imag()=ukfmg[nrb-ir][it].imag();
          }
*/
...
//           mg[ir][it]=um[ir][it]+umg[ir][it];
      ...
      for(it=0;it<lt;it++)
        {
           ug1[ir1][it]=ug[ir1][it];
           um1[ir1][it]=um[ir1][it];
           umg1[ir1][it]=umg[ir1][it];
           mg1[ir1][it]=mg[ir1][it];
        } 
...
int main()
{
   ...
   swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn7>>fn8>>ns>>nr>>lt>>dt>>ds>>dr>>sdep>>rdep>>v>>theta>>boul>>bour>>fmax>>fmin>>lamda>>n>>wd>>r0;
   swq.close();
...
//int deghosting_plus_extrapolation(float **uraw, float **u, complex <float> **uf, complex <float> **ukf, complex <float> **ukfu,  complex <float> **ukfg, complex <float> **ukfm, complex<float> **ukfmg, complex <float> **ufu, complex <float> **ufg, complex <float> **ufm, complex<float>**ufmg,  float **uu, float **ug, float **um, float **umg, float **mg, float *kx, float sdep, float rdep, float ds, float dr, int nr, int lt, int ltt, int boul, int bour,  int nrb, float v, float theta, float ifmin, float ifmax, float lamda, int flag)
   ...
   float **ug1;
   ug1=alloc2float(lt,nr);

   float **um1;
   um1=alloc2float(lt,nr);

   float **umg1;
   umg1=alloc2float(lt,nr);
 
   float **mg1;
   mg1=alloc2float(lt,nr);
  ...
  for(ir=0;ir<nr;ir++)
    dep[ir]=(float)(gwdep[ir]);
 /* 
  for(ir=0;ir<nr;ir++)
    cout<<ir+1<<"   "<<dep[ir]<<endl;
  return 0;
*/
...
//int deghosting_plus_extrapolation(float **uraw, float **u, complex <float> **uf, complex <float> **ukf, complex <float> **ukfu,  complex <float> **ukfg, complex <float> **ukfm, complex<float> **ukfmg, complex <float> **ufu, complex <float> **ufg, complex <float> **ufm, complex<float>**ufmg,  float **uu, float **ug, float **um, float **umg, float **mg, float **ug1,float **um1, float **umg1, float **mg1, float *omega, float *kx, float sdep, float rdep, float ds, float dr, int nr, int lt, int ltt, int boul, int bour,  int nrb, float v, float theta, float ifmin, float ifmax, float lamda, int flag, float wd, float *dep,float r0)
       deghosting_plus_extrapolation(uraw,u, uf, ukf, ukfu, ukfg, ukfm, ukfmg, ufu, ufg, ufm, ufmg,uu,ug, um, umg, mg, ug1,um1,umg1, mg1,omega, kx, sdep, rdep,  ds, dr, nr, lt,  ltt,  boul, bour,  nrb,  v, theta,ifmin, ifmax,  lamda,  flag,wd, dep,r0);
...
/*
       ...
       for(ir=0;ir<nr;ir++)
        for(it=0;it<lt;it++)
         {   
           swq7.write((char*)&um1[ir][it],sizeof(um1[ir][it]));
           swq8.write((char*)&mg1[ir][it],sizeof(mg1[ir][it]));
         }
