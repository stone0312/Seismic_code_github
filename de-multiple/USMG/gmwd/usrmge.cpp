...
/*
               ofstream swqaa;
               swqaa.open("test.dat",ios::binary);
               for(is=0;is<ns;is++)
                  {
                     for(ir=0;ir<nr;ir++)
                       {
                           itmp=is*nr+ir;
                           swqaa.write((char*)&(ur[itmp].real()),sizeof(ur[itmp].real()));
                       }
                  }
                ...
                if(l2norm<err)
                   {
                      for(ix=0;ix<nsr;ix++)
                         {
                            swq5.write((char*)&srmg[ix].real(),sizeof(srmg[ix].real()));
                            swq6.write((char*)&srmg[ix].imag(),sizeof(srmg[ix].imag())); 
                         }
                      for(ix=0;ix<nsr;ix++)
                         desrmg[ix]=us[ix]-srmg[ix];
                      for(ix=0;ix<nsr;ix++)
                         {
                            swq7.write((char*)&desrmg[ix].real(),sizeof(desrmg[ix].real()));
                            swq8.write((char*)&desrmg[ix].imag(),sizeof(desrmg[ix].imag()));
                         }
