...
int     readpar(char *parfn, char *vtruefile, char *vobsfile, char *multiplefile, char *vinvfile, char *vresfile, char *faifile,char *evofile, char *evosubfile,int *nx, int *nz, float * dx, float * dz, int *nitermax, float *lambda, float *niu, float *epsmin,char *type ,float *step, int *n);

void	normalize_data( int nx, int nz, float **data, float amp );

int orientation_smooth_diffusion(float **res, float **mul , float **evo, float **evo_old, float **mulx, float **mult, float **mulx1, float **mult1, float **mulx2, float **mult2, float **tensor, float ** evox, float **evot, float **dm1, float **dm2, float **dm1x, float **dm2t,  float **div, float step, int nx, int lt, float dx, float dt, int nitermax, float epsmin, int n)
{
   int ix, it,ix1,it1,iter;
   float sum1,sum2,tmp1,tmp2;
...
//calculted the first deritive of x and z
   ...
   for(it=0;it<lt;it++)
         mulx[nx-1][it]=-(mul[nx-1][it]-mul[nx-2][it])/dx;     

   for(ix=0;ix<nx;ix++)
     for(it=0;it<lt-1;it++)
         mult[ix][it]=(mul[ix][it]-mul[ix][it+1])/dt;     
   for(ix=0;ix<nx;ix++)
         mult[ix][lt-1]=-(mul[ix][lt-1]-mul[ix][lt-2])/dt;     

  for(ix=0;ix<nx+2*n+1;ix++)
     for(it=0;it<lt+2*n+1;it++)
        {
          mulx1[ix][it]=0.0;
          mult1[ix][it]=0.0;
        }
   ...
   for(ix=n;ix<nx+n;ix++)
     for(it=n;it<lt+n;it++)
        {
          mulx1[ix][it]=mulx[ix-n][it-n];
          mult1[ix][it]=mult[ix-n][it-n];
        }
  ...
  for(ix=0;ix<nx;ix++)
     for(it=0;it<lt;it++)
       {
          tmp1=0.0;
          for(ix1=ix;ix1<ix+2*n+1;ix1++)
            for(it1=it;it1<it+2*n+1;it1++)
               tmp1+=mulx1[ix1][it1]; 
          mulx2[ix][it]=tmp1;
          mulx2[ix][it]/=(float)pow(2*n+1,2);
          ...
          for(ix1=ix;ix1<ix+2*n+1;ix1++)
            for(it1=it;it1<it+2*n+1;it1++)
               tmp2+=mult1[ix1][it1]; 
          mult2[ix][it]=tmp2;
          mult2[ix][it]/=(float)pow(2*n+1,2);
       }

//start the evolution
 
   ofstream swq88;
   swq88.open("./snap.dat",ios::binary);
 ...
 for(iter=0;iter<nitermax;iter++)
  {
   ...
   for(it=0;it<lt;it++)
     for(ix=0;ix<nx-1;ix++)
        evox[ix][it]=(evo[ix][it]-evo[ix+1][it])/dx; 
   for(it=0;it<lt;it++)
        evox[nx-1][it]=-(evo[nx-1][it]-evo[nx-2][it])/dx; 

   for(ix=0;ix<nx;ix++)
     for(it=0;it<lt-1;it++)
         evot[ix][it]=(evo[ix][it]-evo[ix][it+1])/dt; 
   for(ix=0;ix<nx;ix++)
         evot[ix][lt-1]=-(evo[ix][lt-1]-evo[ix][lt-2])/dt; 

   for(ix=0;ix<nx;ix++)
     for(it=0;it<lt;it++)
       {
//calculate the structor tensor
           tensor[0][0]=pow(mulx2[ix][it],2); 
           tensor[0][1]=mulx2[ix][it]*mult2[ix][it]; 
           tensor[1][0]=mulx2[ix][it]*mult2[ix][it]; 
           tensor[1][1]=pow(mult2[ix][it],2); 
...
//calculate the diversity
   ...
   for(it=0;it<lt;it++)
         dm1x[nx-1][it]=-(dm1[nx-1][it]-dm1[nx-2][it])/dx;     

   for(ix=0;ix<nx;ix++)
     for(it=0;it<lt-1;it++)
         dm2t[ix][it]=(dm2[ix][it]-dm2[ix][it+1])/dt;     
   for(ix=0;ix<nx;ix++)
         dm2t[ix][lt]=-(dm2[ix][lt-1]-dm2[ix][lt-2])/dt;     
    ...
    if(iter%50==0)
      {
         for ( ix = 0 ; ix < nx ; ix ++ )
           for ( it = 0 ; it < lt ; it ++ )
              swq88.write((char*)&evo[ix][it],sizeof(evo[ix][it]));
      }
...
int	main( int argc , char *argv[] )
{
	...
	readpar( parfile, vtruefile, vobsfile, multiplefile, vinvfile, vresfile, faifile,evofile, evosubfile,  
		&nx, &nz, &dx, &dz, &nitermax, &lambda, &niu, &epsmin, type, &step, &n);
	...
	float	**mulx2	= NULL;
	float	**mult2	= NULL;
	mulx1	= alloc2float(nz+2*n+1, nx+2*n+1);
	mult1	= alloc2float(nz+2*n+1, nx+2*n+1);
	mulx2	= alloc2float(nz, nx);
	mult2	= alloc2float(nz, nx)
;
//        printf("2222\n");
...
//	write_1d_float_wb(obj, nitermax, "./obj.dat");
        ...
        orientation_smooth_diffusion(vtrue, mul , evo, evo_old, mulx, mult,mulx1, mult1,mulx2, mult2, tensor,  evox, evot, dm1, dm2, dm1x, dm2t,  div,  step,  nx, nz,  dx, dz, nitermax,  epsmin, n);
