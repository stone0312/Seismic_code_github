...
/*
    ...
    gf_all=alloc2complex(lt,ns);
       
    float **gt1;
    gt1=alloc2float(lt,ns);
...
//define the plans and arrays when fftw is complemented
    ...
    float mem;
    mem=0.0;
    mem+=(nsr*lt*4+lt+nx+ns*nr*2+nsr*lt+nx*12+zmax*2)*4.0/1024.0/1024.0;
...
//calculate the omega and kx
   for(it=0;it<lt/2;it++)
      omega[it]=2*pai*it*1000/(2*dt*lt);
   for(it=lt;it<lt;it++)
      omega[it]=2*pai*(-1000/(2*dt)+(it-lt)*1000/(2*dt*lt));

   itmin=int(fmin*2*dt*lt/1000);
   itmax=int(fmax*2*dt*lt/1000)+1;
    ...
    for(it=0;it<lt;it++)    
        {
          ...
          else if(it>itmax&&it<lt)  //it>itmax&&it<lt-itmax+itmin
            {
               for(is=0;is<nsr;is++)
                 mf[is][it]=(0.0,0.0);
            } 
