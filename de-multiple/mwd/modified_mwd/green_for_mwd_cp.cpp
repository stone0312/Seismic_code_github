...
//reflectivity for horizonial layer
    ...
    float **amp;
    amp=alloc2float(ltt,nx);

    complex <float> **g_all1;
    g_all1=alloc2complex(ltt,nx);
...
*/
          ...
          for(is=0;is<nx;is++)
            {
               s_k[is].real()=out4[is][0]/nx;
               s_k[is].imag()=out4[is][1]/nx;
            }
...
//final Green's functions in kx-w domain for a certain (kx,w) pair;
    ...
    cout<<"Calculation of Green's Function for Positive Frequency in kx-omega domain done!"<<endl;      
    cout<<"Calculation of Green's Function for Necgtive Frequency in kx-omega domain starts:"<<endl;              

    for(it=ltt/2+1;it<ltt;it++)
       { 
          g_all[0][it].real()=g_all[0][ltt-it].real();
          g_all[0][it].imag()=-g_all[0][ltt-it].imag();
       }
    for(it=ltt/2+1;it<ltt;it++)
       {
          for(is=1;is<nx;is++)
             {
                g_all[is][it].real()=g_all[nx-is][ltt-it].real();
                g_all[is][it].imag()=-g_all[nx-is][ltt-it].imag();
             }
       }

/*
    cout<<"======================================================="<<endl;
    cout<<"(100,200)"<<g_all[100][200].real()<<endl;
    cout<<"(nx-100,ltt-200)"<<g_all[nx-100][ltt-200].real()<<endl;
    cout<<"(100,200)"<<g_all[100][200].imag()<<endl;
    cout<<"(nx-100,ltt-200)"<<g_all[nx-100][ltt-200].imag()<<endl;
    cout<<"======================================================="<<endl;
*/
    ...
    ofstream swq8;
    swq8.open("/data2/swq/x_omega_domain_green_function_real.dat",ios::binary);

    ofstream swq9;
    swq9.open("/data2/swq/x_omega_domain_green_function_imag.dat",ios::binary);
...
//ifft to transform the Green's function from x-omega domain to x-t domain 
       for(it=0;it<ltt;it++)
          {
          for(is=0;is<nx;is++)
            {
               in5[is][0]=g_all[is][it].real();
               in5[is][1]=g_all[is][it].imag();
            }
          fftwf_execute(p5);

          for(is=0;is<nx;is++)
            {
               g_all1[is][it].real()=out5[is][0];
               g_all1[is][it].imag()=out5[is][1];
            }
          }
    
       for(is=0;is<nx;is++)  
          for(it=0;it<ltt;it++)
              amp[is][it]=sqrt(g_all[is][it].real()*g_all[is][it].real()+g_all[is][it].imag()*g_all[is][it].imag());

       ofstream swq88;
       swq88.open("/data2/swq/Green_Function_Amp.dat",ios::binary);
       for(is=0;is<nx;is++)
         for(it=0;it<ltt;it++)
             swq88.write((char*)&amp[is][it],sizeof(amp[is][it]));
       swq88.close();   
       
//       for(it=0;it<ltt;it++)
//          cout<<it<<"  "<<amp[nx/2][it]<<endl;
          ...
          for(is=0;is<nx;is++)
            {
               for(it=0;it<ltt;it++)
                  {          
                    in2[it][0]=g_all1[is][it].real();
                    in2[it][1]=g_all1[is][it].imag();
                  }
