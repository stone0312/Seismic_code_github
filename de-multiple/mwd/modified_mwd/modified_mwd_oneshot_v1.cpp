...
int main()
{
   ...
   int is,ir,it,iz,is1,ir1,is2,ir2,itmp,ix,tmp,tmp1;
    ...
    ifstream swq;
    swq.open("modified_mwd_oneshot.par");
    if(!swq)
      {
          cout<<"cannot open modified_mwd_oneshot.par"<<endl;
          abort();
      }
...
/*
             ofstream swq88;
             swq88.open("test_seekg.dat",ios::binary);

             for(is=0;is<ns;is++)
               for(ir=0;ir<nr;ir++) 
                   swq88.write((char*)&(d[is][ir].real()),sizeof(d[is][ir].real()));
             swq88.close();
             return 0;
*/              
...
*/
                     if((in7==NULL)||(out7==NULL))
                     ...
                     else
                       {
                         for(is=0;is<nxb;is++)
                            {
                               in7[is][0]=gs_for[is][iz].real();
                               in7[is][1]=gs_for[is][iz].imag();
                            }
                       }
...
*/
                    ...
                    for(is=0;is<nxb;is++)
                     {
                       gs_for[is][iz].real()=out8[is][0]/sqrt(nxb);
                       gs_for[is][iz].imag()=out8[is][1]/sqrt(nxb);
                     }
...
/*
                 ...
                 for(iz=1;iz<zmax;iz++)
                    for(is=0;is<nxb;is++)
                        cout<<"(is,iz)====("<<is<<","<<iz<<")===="<<gs_for[is][iz]<<endl;
                 return 0;
*/
                 ...
                 for(iz=1;iz<zmax-1;iz++)
                   {
                      ...
                      if((in5==NULL)||(out5==NULL))
                      ...
                      else
                        {
                            for(is=0;is<nxb;is++)
                              {
                                 in5[is][0]=gs_back[is][iz].real();
                                 in5[is][1]=gs_back[is][iz].imag();
                              }
                        }
                     ...
                     for(is=0;is<nxb;is++)
                       {
                            uxf[is].real()=out5[is][0]/sqrt(nxb);
                            uxf[is].imag()=out5[is][1]/sqrt(nxb);
                       }
...
*/
                   ...
                   for(is=0;is<nxb;is++)
                      {
                         gs_back[is][iz].real()=out6[is][0]/sqrt(nxb);
                         gs_back[is][iz].imag()=out6[is][1]/sqrt(nxb);
                      }
...
/*
                for(iz=1;iz<zmax;iz++)
                    for(is=0;is<nxb;is++)
                        cout<<"(is,zmax)====("<<is<<","<<iz<<")===="<<gs_back[is][iz]<<endl;
                 return 0;
*/                   
...
/*
             ...
             for(is2=0;is2<nx;is2++)
                for(is=0;is<nxb;is++)
                   cout<<"(shot,receiver)====("<<is2<<","<<is<<")===="<<g_k[is2][is]<<endl;
             return 0;
*/
...
//calculating the predicted results in x-w domain;
               for(is1=0;is1<nx;is1++)
                ...
                for(is1=0;is1<nx;is1++)                    
                   gs[is1]=g[shot-1][is1*int(dsx/dx)+bou]+A[is1]; 
