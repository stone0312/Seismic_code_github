...
int main()
{
   ...
   int is,ir,it,iz,is1,ir1,is2,ir2,itmp;
    ...
    swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn6>>fn7>>fn8>>fn9>>nx>>dx>>dz>>ns>>nr>>lt>>dt>>dsx>>drx>>fmin>>fmax>>v>>bou>>shot;
    swq.close();
...
//calculate the omega and kx
   ...
   cout<<"totally "<<(itmax-itmin)<<" frequency slices need to be calculated"<<endl;
    ...
    float *hann;
    hann=al3c1float(itmax-itmin+1);
    for(it=0;it<int((itmax-itmin+1)*2/3);it++)
       hann[it]=1.0;
    for(it=int((itmax-itmin+1)*2/3);it<itmax-itmin+1;it++)
       hann[it]=cos(0.5*pai*(it-int((itmax-itmin+1)*2/3))/(itmax-itmin-int((itmax-itmin+1)*2/3)));
           ...
           for(is2=0;is2<nx;is2++)
            {
              for(is=0;is<nxb;is++)
                 s[is]=(0.0,0.0);
              s[is2*int(dsx/dx)+bou].real()=1.0*hann[it-itmin];
