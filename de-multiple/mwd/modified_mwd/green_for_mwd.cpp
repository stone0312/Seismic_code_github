...
//calculating the Green's functions in kx-w domain;
  for(is2=0;is2<nx-2*bou;is2++)
   {
    cout<<is2+1<<" th shot begins..."<<endl; 
      ...
      for(is=0;is<nx;is++)
         {
             for(it=ltt/2+1;it<ltt;it++)
                 {
                    g_all1[is][it].real()=g_all1[is][ltt-it].real();
                    g_all1[is][it].imag()=-g_all1[is][ltt-it].imag();
                 }
         }
...
//ifft to transform the Green's function from x-omega domain to x-t domain 
        ...
        for(is=0;is<nx-2*bou;is++)
           for(it=0;it<lt;it++)        
             swq2.write((char*)&g_final[is+bou][it],sizeof(g_final[is+bou][it]));  
