...
int main()
{
   ...
   int is,ir,it,iz,is1,ir1,is2,ir2;
    ...
    ifstream swq;
    swq.open("modified_mwd_v5.par");
    if(!swq)
      {
          cout<<"cannot open modified_mwd_v5.par"<<endl;
          abort();
      }
...
//ordering the depth of water layer to find the maximum depth
    ...
    complex <float> *gs_k;
    gs_k=alloc1complex(nxb);  
    complex <float> *gr_k;
    gr_k=alloc1complex(nxb);
...
//define the plans and arrays when fftw is complemented
    ...
    p5=fftwf_plan_dft_1d(nxb,in5,out5,FFTW_FORWARD,FFTW_MEASURE);
    p6=fftwf_plan_dft_1d(nxb,in3,out3,FFTW_BACKWARD,FFTW_MEASURE);

    float mem;
    mem=0.0;
    mem+=(nsr*ltt*3+ltt+nx*4+ns*nr*2+nxb*16+zmax*nxb*4)*4.0/1024.0/1024.0;
                    ...
                    for(ir=0;ir<nx;ir++)
                       {
                          ...
                          for(ir1=0;ir1<nx;ir1++)
                             gr[ir1]=gr[ir1+bou]+B[ir1];
                       
                          for(ir1=0;ir1<nx;ir1++)
                             {
                                for(is1=0;is1<nx;is1++)
                                   gsd[ir1]+=gs[is1+bou]*d[is1][ir1];
                             } 
                           for(ir1=0;ir1<nx;ir1++)
