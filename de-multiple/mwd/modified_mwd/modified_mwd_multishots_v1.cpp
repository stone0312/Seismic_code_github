...
int main()
{
   ...
   int is,ir,it,iz,is1,ir1,is2,ir2,itmp;
    ...
    if(!swq)
      {
          cout<<"modified_mwd_multishots.par"<<endl;
          abort();
      }
...
/*
   int tmp;
   tmp=itmax-int((itmax-itmin)/5);
   float *ham;
   ham=alloc1float(itmax);
   for(it=0;it<itmax;it++)
      ham[it]=1.0;
   for(it=tmp;it<itmax;it++)
      ham[it]=0.5+0.5*cos(pai*(it-tmp)/int((itmax-itmin)/5));
 */  
               ...
               for(is=0;is<ns;is++)
                 {
                   for(ir=0;ir<nr;ir++)
                     {
                        itmp=is*nr+ir;
                        d[is][ir].real()=dd[itmp].real()*ham[it];
                        d[is][ir].imag()=dd[itmp].imag()*ham[it];
                     }
                 }
           ...
           for(is2=0;is2<nx;is2++)
            {
              ...
              if((in4==NULL)||(out4==NULL))
              ...
              else
                {
                  for(is=0;is<nxb;is++)
                    {
                       in4[is][0]=s[is].real()*ham[it];
                       in4[is][1]=s[is].imag()*ham[it];
                    }
                }
