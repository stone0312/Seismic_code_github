...
int main(int argc,char **argv)
{
   ...
   MPI_Comm_rank(MPI_COMM_WORLD,&myid);
    ...
    ifstream swq;
    swq.open("modified_mwd_multishots_mpi.par");
    if(!swq)
      {
          cout<<"cannot open modified_mwd_multishots.par"<<endl;
          abort();
      }
...
/*
               ofstream swq2b;
               swq2b.open("check_read_real.dat",ios::binary);
               ofstream swq2bb;
               swq2bb.open("check_read_imag.dat",ios::binary);
               for(is=0;is<ns;is++)
                 {
                   for(ir=0;ir<nr;ir++)
                     { 
                        swq2b.write((char*)&(d[is][ir].real()),sizeof(d[is][ir].real()));
                        swq2bb.write((char*)&(d[is][ir].imag()),sizeof(d[is][ir].imag()));
                     }
                 }
               swq2b.close();
               swq2bb.close();
              ...
              for(is2=0;is2<nx;is2++)
                {
                  for(is1=0;is1<nx;is1++)
                     gs[is1]=g[is2][is1*int(dsx/dx)+bou];
                  ...
                  for(ir=0;ir<nx;ir++)
                    {
                       ...
                       for(ir1=0;ir1<ir+1;ir1++)
                        {
						   gtmp2+=d[is2][ir1]*g[ir][ir1*int(drx/dx)+bou];
				           gtmp3+=d[ir1][ir]*gs[ir1];
						}
					
					   mf=gtmp2+gtmp3;
/*
                       mf.real()=mf.real()/(ir+1);
                       mf.imag()=mf.imag()/(ir+1);
*/
                       swq6.write((char*)&mf.real(),sizeof(mf.real()));
                       swq7.write((char*)&mf.imag(),sizeof(mf.imag()));
