...
//ordering the depth of water layer to find the maximum depth
    ...
    float *mir_dep;
    mir_dep=alloc1float(nx);
 
   int *receiver;
    receiver=alloc1int(nx);

   int *mir_receiver;
    mir_receiver=alloc1int(nx);
           ...
           for(is2=0;is2<nx;is2++)
            {
              for(ix=0;ix<nx;ix++)
               {
                 k=(dep1[ix+is2+1]-dep1[ix+is2])/dx;
                 theta=atan(k);

                 k1=dep1[ix+is2]/(ix*dx);
                 fai=atan(k1);

                 mir_k=tan(2*theta-fai);

                 if(-(float(dep1[ix+is2])-float(mir_k*(ix+is2)*dx))/mir_k/dx>=0.0)
                   receiver[ix]=int(-(float(dep1[ix+is2])-float(mir_k*ix*dx))/mir_k/dx+0.5);
                 else
                   receiver[ix]=int(-(float(dep1[ix+is2])-float(mir_k*ix*dx))/mir_k/dx-0.5);

                 mir_dep[ix]=fabs((sqrt(dep1[ix+is2]*dep1[ix+is2]+(ix*dx-receiver[ix]*dx)*(ix*dx-receiver[ix]*dx))+sqrt(dep1[ix+is2]*dep1[ix+is2]+ix*dx*ix*dx))*sin(fai));

                if(cos(fai)>=0)
                  mir_receiver[ix]=int(fabs((sqrt(dep1[ix+is2]*dep1[ix+is2]+(ix*dx-receiver[ix]*dx)*(ix*dx-receiver[ix]*dx))+sqrt(dep1[ix+is2]*dep1[ix+is2]+ix*dx*ix*dx))*cos(fai)/dx)+0.5);
                else
                  mir_receiver[ix]=int(fabs((sqrt(dep1[ix+is2]*dep1[ix+is2]+(ix*dx-receiver[ix]*dx)*(ix*dx-receiver[ix]*dx))+sqrt(dep1[ix+is2]*dep1[ix+is2]+ix*dx*ix*dx))*cos(fai)/dx-0.5));

//                cout<<"Reflection Point==== "<<ix<<", Ishot==== "<<is2<<" ,Receiver==== "<<receiver[ix]<<" ,Mir_Depth==== "<<mir_dep[ix]<<" ,Mir_Receiver==== "<<mir_receiver[ix]<<endl; 
...
//final Green's functions in kx-w domain for a certain (kx,w) pair;
                 ...
                 for(is=0;is<nxb;is++)
                  ...
                  if(tmp1>-1&&tmp1<nx+bou)
                    g[is2][tmp]+=g_tmp[tmp1];  
                  else
                    g[is2][tmp]+=(0.0,0.0);
                 }//1111  
                 else
