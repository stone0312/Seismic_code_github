...
int main()
{
   ...
   int is,ir,it,iz,is1,ir1,is2,ir2;
    ...
    ifstream swq;
    swq.open("modified_mwd_v10.par");
    if(!swq)
      {
          cout<<"cannot open modified_mwd_v10.par"<<endl;
          abort();
      }
    swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn6>>fn7>>fn8>>fn9>>nx>>dx>>dz>>ns>>nr>>lt>>dt>>dsx>>drx>>fmin>>fmax>>v>>bou;
    swq.close();
...
//start to compute the predicticted model of the negative frequency
       ...
       if(!swq77)
        ...
        cout<<"Negative frequcies prediction start..."<<endl;
