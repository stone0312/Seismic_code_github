...
int main()
{
   ...
   int is,ir,it,iz,is1,ir1,is2,ir2,itmp;
    ...
    ifstream swq;
    swq.open("modified_mwd_Ricker_oneshot.par");
    if(!swq)
      {
          cout<<"cannot open modified_mwd_Ricker_oneshot.par"<<endl;
          abort();
      }
    swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn6>>fn7>>fn8>>fn9>>nx>>dx>>dz>>ns>>nr>>lt>>dt>>dsx>>drx>>fmin>>fmax>>v>>bou>>shot>>fm>>tm;
    swq.close();
