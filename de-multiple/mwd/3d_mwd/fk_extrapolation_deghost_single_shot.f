      ...
      INTEGER,PARAMETER::lt=
	  INTEGER,PARAMETER::nx=
	  REAL,PARAMETER::z_line=
	  REAL,PARAMETER::dx=
	  REAL,PARAMETER::dt=
	  REAL,PARAMETER::vmin=
	  REAL,PARAMETER::r_coe=
	  REAL,PARAMETER::delt=
      CHARACTER*256::FN1=''
	  CHARACTER*256::FN2=''
	  CHARACTER*256::FN3=''
!************************************************
!     lt: the number of time sampling
!     nx: the number of receovers of one shot
! z_line: the depth of streamer         (m)
!     dx: the interval of receivers     (m)
!     dt: the interval of time sampling (s)
!   vmin: the velocity of shallow sea water (m/S)
!  r_coe: the reflection coefficient of sea surface
!   delt: the regular factor to stablize (+)
!    FN1: input the original single shot gather
!    FN2: output the up_field
!    FN3: output the ghost_field
!**************************************************

	  INTEGER itrace,it
      
	  REAL ww(lt)                 ! angle frequency
	  REAL kk(nx)                 ! horizontal wavenumber
	  REAL  single_shot(lt,nx)
	  REAL  up_field(lt,nx)
	  REAL  ghost_field(lt,nx)
	  COMPLEX csingle_shot(lt,nx)
	  COMPLEX cup_field(lt,nx)
	  COMPLEX cghost_field(lt,nx)

	  REAL    phase(lt,nx)
      COMPLEX deghost_operator(lt,nx)
!     ***********************************************
!
!     input the original single_shot and transform to
!      f-k domain
!
!     **********************************************

      OPEN(51,FILE=FN1,ACTION='READ',FORM='BINARY')
	  READ(51)single_shot
	  CLOSE(51)

      DO itrace=1,nx
	   DO it=1,lt
	     csingle_shot(it,itrace)=CMPLX(single_shot(it,itrace))
	   END DO
	  END DO


	  DO itrace=1,nx
	   CALL FFT1D(csingle_shot(:lt,itrace),lt,1)
	  END DO

	  DO it=1,lt
	   CALL FFT1D(csingle_shot(it,:nx),nx,1)
	  END DO

!     *************************************************
!
!     begin f-k extrapolation of csp to get up_field
!
!     *************************************************

      DO it=1,lt/2+1
       ww(it)=2*pi*(it-1)*(1.0/(lt*dt))
	  END DO
	  ...
	  DO itrace=1,nx/2+1
	   kk(itrace)=(itrace-1)*(1.0/(dx*nx))
	  END DO

	  DO itrace=nx/2+2,nx
       kk(itrace)=-kk(nx+2-itrace)
	  END DO

	  DO it=1,lt
	   DO itrace=1,nx
         IF(((ww(it)/vmin)**2-(kk(itrace))**2)<0.0)THEN
			 phase(it,itrace)=0.0
			 deghost_operator(it,itrace)=CMPLX(0,0)
		 ELSE
			 phase(it,itrace)=2*z_line*
     +     SQRT((ww(it)/vmin)**2-(kk(itrace))**2)
	         deghost_operator(it,itrace)=1+delt+r_coe*
     +      CMPLX(COS(phase(it,itrace)),-SIN(phase(it,itrace)))
	     END IF
	   END DO
	  END DO
	  ...
	  DO itrace=1,nx
	   DO it=1,lt
        ...
        cup_field(it,itrace)=csingle_shot(it,itrace)/
     +    deghost_operator(it,itrace)
	  ...
	  DO itrace=1,nx
       ...
       CALL FFT1D(cup_field(:lt,itrace),lt,-1)

	  END DO

	  DO it=1,lt

       CALL FFT1D(cup_field(it,:nx),nx,-1)

	  END DO

      OPEN(52,FILE=FN2,ACTION='WRITE',FORM='BINARY')
	  WRITE(52)REAL(cup_field)
      CLOSE(52)

!*************************************************************
!
!     begin f-k extrapolation of csp to get ghost_field
!
!*************************************************************
     ...
     +   SQRT((ww(it)/vmin)**2-(kk(itrace))**2)
	    ...
	    deghost_operator(it,itrace)=r_coe-delt+
     +  CMPLX(COS(phase(it,itrace)),SIN(phase(it,itrace)))
        
	    cghost_field(it,itrace)=csingle_shot(it,itrace)/
     +  deghost_operator(it,itrace)

	   END DO
	  END DO
     
	  OPEN(52,FILE=FN3,ACTION='WRITE',FORM='BINARY')
	  WRITE(52)REAL(cghost_field)
	  CLOSE(52)
