...
int main()
{
   char fn1[256],fn2[256],fn3[256],fn4[256],fn5[256],fn6[256],fn7[256],fn8[256],fn9[256],fn10[256],fn100[256],fninmcg[256],fnoutmcg[256];
   int ns,nr,lt,ltt,boul,bour, ifmin, ifmax, flag,nrb, n,irmcg;
   float ds,dr,dt,sdep, rdep,theta, v, fmax, fmin, lamda,wd, min,max;
 
   ifstream swq;
   swq.open("wlrim_prediction_mcg.par");
   swq>>fn1>>fn2>>fn3>>fn4>>fn5>>fn6>>fn7>>fn8>>fn9>>fn10>>fn100>>fninmcg>>fnoutmcg>>ns>>nr>>lt>>dt>>ds>>dr>>sdep>>rdep>>v>>theta>>boul>>bour>>fmax>>fmin>>wd>>min>>max>>irmcg;
   swq.close();
   ...
   int it,it1,is,ir,is1,ir1,is2,ir2,ir3;
   ...
   complex<float> **inmcg;
   inmcg=alloc2complex(ltt,ns*nr);

   for(ir=0;ir<ns*nr;ir++)
     for(it=0;it<ltt;it++)
        inmcg[ir][it]=(0.0,0.0);

   float **inmcgt;
   inmcgt=alloc2float(ltt,ns*nr);
   float **outmcgt;
   outmcgt=alloc2float(lt,nr);
  ...
  ofstream swqin;
  swqin.open(fninmcg,ios::binary);
  if(!swqin)
       cout<<"cannot open "<<fninmcg<<endl;

  ofstream swqout;
  swqout.open(fnoutmcg,ios::binary);
  if(!swqout)
       cout<<"cannot open "<<fnoutmcg<<endl;
...
//                          for(ir2=0;ir2<nr;ir2++) //G Loop
                            {
                              inmcg[ir1*nr+ir2][it]=usf[ir2][it]*urf[ir1][it]*gf[ir2][it];               
                              imf[ir][it]+=usf[ir2][it]*urf[ir1][it]*gf[ir2][it];  
                            }                         
                       }//1111
...
//                         for(ir2=0;ir2<nr;ir2++) //G Loop
           ...
           for(ir2=0;ir2<ns*nr;ir2++)
             for(it=ltt/2+1;it<ltt;it++)
                {
                   inmcg[ir2][it].real()=inmcg[ir2][ltt-it].real();
                   inmcg[ir2][it].imag()=-inmcg[ir2][ltt-it].imag();
                }
            ...
            for(ir2=0;ir2<ns*nr;ir2++)
              {
                 for(it=0;it<ltt;it++)
                  {
                    in6[it][0]=inmcg[ir2][it].real();
                    in6[it][1]=inmcg[ir2][it].imag();
                  }
                 fftwf_execute(p6);

                 for(it=0;it<ltt;it++)
                   inmcgt[ir2][it]=out6[it][0];

                 for(it=0;it<lt;it++)
                    swqin.write((char*)&inmcgt[ir2][it],sizeof(inmcgt[ir2][it]));
              }
                 ...
                 for(it=0;it<lt;it++)
                  {
                     for(ir3=0;ir3<nr;ir3++)
                       outmcgt[ir2][it]+=inmcgt[ir2*nr+ir3][it];
                  }

                 
                 for(it=0;it<lt;it++)
                    swqout.write((char*)&outmcgt[ir2][it],sizeof(outmcgt[ir2][it]));
              }
