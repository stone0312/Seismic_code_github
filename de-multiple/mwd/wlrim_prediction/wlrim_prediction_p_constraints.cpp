...
int main()
{
   ...
   int max_ipr,max_ips,tmp1,tmp2,tmp3;
       ...
       for(ir2=0;ir2<nr;ir2++)
         {
           ...
           pr[is*nr+ir2]=max_ipr-ir2*np;

         } 
      }  

  cout<<"CSG Green's Semblance Constraints Done..."<<endl;

  for(ir=0;ir<nr;ir++)
    {
      for(ir2=0;ir2<nr*np;ir2++)
        for(it1=0;it1<lt;it1++)
         swq8.read((char*)&sems[ir2][it1],sizeof(sems[ir2][it1]));
           ...
           for(ir3=ir2*np;ir3<(ir2+1)*np;ir3++)
             {
               for(it1=0;it1<lt;it1++)
                 {
                     if(sems[ir3][it1]>maxsems)
                        {
                           maxsems=sems[ir3][it1];
                           max_ips=ir3;
                        }
                 }
              }

           ps[ir*ns+ir2]=max_ips-ir2*np;

         } 
      }  

  cout<<"CRG Green's Semblance Constraints Done..."<<endl;
                       ...
                       for(ir2=0;ir2<nr;ir2++)
                         {
                           ...
                           for(ir3=ir2*np;ir3<ir2*np+tmp3;ir3++)
                            for(it1=0;it1<ltt;it1++)
                              gpf[ir3][it1]=(0.0,0.0);

                           for(ir3=ir2*np+tmp3+1;ir3<(ir2+1)*np;ir3++)
                            for(it1=0;it1<ltt;it1++)
                              gpf[ir3][it1]=(0.0,0.0);
                         }
