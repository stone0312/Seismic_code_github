...
int main()
{
   ...
   float *final_tmp=new float [2*length-1];
   float **toep=new float *[length];
   for(i=0;i<length;i++)
       toep[i]=new float [length];
   float *toep1d=new float [length*length];
...
*/
         ...
         gaus(toep1d,lag_right,length);
         convolution(sintrace,length,lag_right,length,final_tmp);

         for(j=0;j<lag;j++)
             u_pre[i][j]=0.0;
         for(j=lag;j<length;j++)
    ...
    ofstream swq4;
    swq4.open("multiple_predicted_by_deconvolution_lag190_length1200.dat",ios::binary);
    if(!swq4)
    {
        cout<<"cannot open"<<endl;
        abort();
